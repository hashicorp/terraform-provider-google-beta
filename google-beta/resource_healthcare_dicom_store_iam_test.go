...
func TestAccHealthcareDicomStoreIamBinding(t *testing.T) {
	...
	dicomStoreName := fmt.Sprintf("tf-test-%s", acctest.RandString(10))
		...
		Steps: []resource.TestStep{
			{
				...
				Check: testAccCheckGoogleHealthcareDicomStoreIamBindingExists("foo", roleId, []string{
		...
		Steps: []resource.TestStep{
			{
				...
				Check: testAccCheckGoogleHealthcareDicomStoreIamMemberExists("foo", roleId,
					fmt.Sprintf("serviceAccount:%s@%s.iam.gserviceaccount.com", account, projectId),
				),
			},
			{
		...
		Steps: []resource.TestStep{
			{
				...
				Check: testAccCheckGoogleHealthcareDicomStoreIamPolicyExists("foo", roleId,
					fmt.Sprintf("serviceAccount:%s@%s.iam.gserviceaccount.com", account, projectId),
				),
			},
			{
...
func testAccCheckGoogleHealthcareDicomStoreIamBindingExists(bindingResourceName, roleId string, members []string) resource.TestCheckFunc {
...
func testAccCheckGoogleHealthcareDicomStoreIamMemberExists(n, role, member string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
func testAccCheckGoogleHealthcareDicomStoreIamPolicyExists(n, role, policy string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
