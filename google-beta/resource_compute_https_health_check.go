...
func resourceComputeHttpsHealthCheckCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return err
	} else if !isEmptyValue(reflect.ValueOf(hostProp)) {
		obj["host"] = hostProp
	}
	...
	if err != nil {
		return err
	} else if !isEmptyValue(reflect.ValueOf(portProp)) {
		obj["port"] = portProp
	}
	requestPathProp, err := expandComputeHttpsHealthCheckRequestPath(d.Get("request_path"), d, config)
	if err != nil {
		return err
	} else if !isEmptyValue(reflect.ValueOf(requestPathProp)) {
...
func resourceComputeHttpsHealthCheckRead(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := replaceVars(d, config, "https://www.googleapis.com/compute/beta/projects/{{project}}/global/httpsHealthChecks/{{name}}")
	if err != nil {
		return err
	}
