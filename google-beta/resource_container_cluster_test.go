...
func testAccContainerCluster_updateMeshCertificatesConfig(projectID string, clusterName string, enable bool) string {
	meshCertificatesConfig := ""
	if enable {
		meshCertificatesConfig = `
			workload_identity_config {
			workload_pool = "${data.google_project.project.project_id}.svc.id.goog"
...
`, projectID, clusterName, meshCertificatesConfig)
}
