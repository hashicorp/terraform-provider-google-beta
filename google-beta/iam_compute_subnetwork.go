...
func (u *ComputeSubnetworkIamUpdater) qualifySubnetworkUrl(methodIdentifier string) string {
	return fmt.Sprintf("https://www.googleapis.com/compute/beta/%s/%s", fmt.Sprintf("projects/%s/regions/%s/subnetworks/%s", u.project, u.region, u.subnetwork), methodIdentifier)
}
