					...
					Schema: map[string]*schema.Schema{
						"networks": {
							...
							Set: func(v interface{}) int {
								...
								schema.SerializeResourceForHash(&buf, raw, resourceDnsManagedZone().Schema["private_visibility_config.networks"].Elem.(*schema.Resource))
...
func flattenDnsManagedZoneForwardingConfigTargetNameServers(v interface{}, d *schema.ResourceData) interface{} {
	...
	l := v.([]interface{})
	transformed := schema.NewSet(schema.HashResource(dnsManagedZoneForwardingConfigTargetNameServersSchema()), []interface{}{})
	for _, raw := range l {
		original := raw.(map[string]interface{})
		if len(original) < 1 {
			// Do not include empty json objects coming back from the api
