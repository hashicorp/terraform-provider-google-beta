...
func init() {
	resource.AddTestSweepers("HealthcareFhirStore", &resource.Sweeper{
		Name: "HealthcareFhirStore",
		F:    testSweepHealthcareFhirStore,
	})
}

// At the time of writing, the CI only passes us-central1 as the region
func testSweepHealthcareFhirStore(region string) error {
	resourceName := "HealthcareFhirStore"
	...
	listTemplate := strings.Split("https://healthcare.googleapis.com/v1beta1/{{dataset}}/fhirStores?fhirStoreId={{name}}", "?")[0]
	...
	resourceList, ok := res["fhirStores"]
	if !ok {
	...
	for _, ri := range rl {
		...
		deleteTemplate := "https://healthcare.googleapis.com/v1beta1/{{dataset}}/fhirStores/{{name}}"
