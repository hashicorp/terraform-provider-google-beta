...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionOSConfigV2PolicyOrchestrator,
		DeleteFunction: testSweepOSConfigV2PolicyOrchestrator,
	}
...
func testSweepOSConfigV2PolicyOrchestrator(_ string) error {
	return listAndActionOSConfigV2PolicyOrchestrator(deleteResourceOSConfigV2PolicyOrchestrator)
}

func listAndActionOSConfigV2PolicyOrchestrator(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "OSConfigV2PolicyOrchestrator"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://osconfig.googleapis.com/v2beta/projects/{{project}}/locations/global/policyOrchestrators", "?")[0]
		...
		resourceList, ok := res["policyOrchestrators"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'policyOrchestrators'")
		} else {
...
func deleteResourceOSConfigV2PolicyOrchestrator(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://osconfig.googleapis.com/v2beta/projects/{{project}}/locations/global/policyOrchestrators/{{policy_orchestrator_id}}"
