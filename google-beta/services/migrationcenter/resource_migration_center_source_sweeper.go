...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMigrationCenterSource,
		DeleteFunction: testSweepMigrationCenterSource,
	}
...
func testSweepMigrationCenterSource(_ string) error {
	return listAndActionMigrationCenterSource(deleteResourceMigrationCenterSource)
}

func listAndActionMigrationCenterSource(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MigrationCenterSource"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/sources", "?")[0]
		...
		resourceList, ok := res["sources"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'sources'")
		} else {
...
func deleteResourceMigrationCenterSource(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/sources/{{source_id}}"
