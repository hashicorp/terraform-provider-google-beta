...
func init() {
	registry.FrameworkListResource{
		Name:        "google_migration_center_preference_set",
		ProductName: "MigrationCenter",
		Func:        NewMigrationCenterPreferenceSetListResource,
	}.Register()
}

var _ list.ListResource = &MigrationCenterPreferenceSetListResource{}

type MigrationCenterPreferenceSetListResource struct {
	tpgresource.ListResourceMetadata
}

func NewMigrationCenterPreferenceSetListResource() list.ListResource {
	listR := &MigrationCenterPreferenceSetListResource{}
	listR.TypeName = "google_migration_center_preference_set"
	listR.SDKv2Resource = ResourceMigrationCenterPreferenceSet()
...
// MigrationCenterPreferenceSetListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type MigrationCenterPreferenceSetListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *MigrationCenterPreferenceSetListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data MigrationCenterPreferenceSetListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListMigrationCenterPreferenceSets(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "preference_set_id"); err != nil {
					return err
				}
...
func ListMigrationCenterPreferenceSets(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceMigrationCenterPreferenceSet().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{MigrationCenterBasePath}}projects/{{project}}/locations/{{location}}/preferenceSets")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceMigrationCenterPreferenceSet(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "preferenceSets",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["preferenceSetId"]; ok && v != nil {
				if err := d.Set("preference_set_id", v); err != nil {
					return fmt.Errorf("error setting preference_set_id: %w", err)
				}
			}
			if err = ResourceMigrationCenterPreferenceSetFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
