...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMigrationCenterImportJob,
		DeleteFunction: testSweepMigrationCenterImportJob,
	}
...
func testSweepMigrationCenterImportJob(_ string) error {
	return listAndActionMigrationCenterImportJob(deleteResourceMigrationCenterImportJob)
}

func listAndActionMigrationCenterImportJob(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MigrationCenterImportJob"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/importJobs", "?")[0]
		...
		resourceList, ok := res["importJobs"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'importJobs'")
		} else {
...
func deleteResourceMigrationCenterImportJob(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/importJobs/{{import_job_id}}"
