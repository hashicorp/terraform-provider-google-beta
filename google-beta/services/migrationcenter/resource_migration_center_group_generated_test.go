...
func testAccMigrationCenterGroup_migrationGroupBasicExample(context map[string]interface{}) string {
...
func testAccCheckMigrationCenterGroupDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_migration_center_group" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(migrationcenter.Product, config)+"projects/{{project}}/locations/{{location}}/groups/{{group_id}}")
			if err != nil {
				return err
			}
			...
			if err == nil {
				return fmt.Errorf("MigrationCenterGroup still exists at %s", url)
			}
		}
