...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMigrationCenterDiscoveryClient,
		DeleteFunction: testSweepMigrationCenterDiscoveryClient,
	}
...
func testSweepMigrationCenterDiscoveryClient(_ string) error {
	return listAndActionMigrationCenterDiscoveryClient(deleteResourceMigrationCenterDiscoveryClient)
}

func listAndActionMigrationCenterDiscoveryClient(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MigrationCenterDiscoveryClient"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/discoveryClients", "?")[0]
		...
		resourceList, ok := res["discoveryClients"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'discoveryClients'")
		} else {
...
func deleteResourceMigrationCenterDiscoveryClient(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/discoveryClients/{{discovery_client_id}}"
