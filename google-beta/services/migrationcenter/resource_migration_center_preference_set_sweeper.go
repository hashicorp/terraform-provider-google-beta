...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMigrationCenterPreferenceSet,
		DeleteFunction: testSweepMigrationCenterPreferenceSet,
	}
...
func testSweepMigrationCenterPreferenceSet(_ string) error {
	return listAndActionMigrationCenterPreferenceSet(deleteResourceMigrationCenterPreferenceSet)
}

func listAndActionMigrationCenterPreferenceSet(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MigrationCenterPreferenceSet"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/preferenceSets", "?")[0]
		...
		resourceList, ok := res["preferenceSets"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'preferenceSets'")
		} else {
...
func deleteResourceMigrationCenterPreferenceSet(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://migrationcenter.googleapis.com/v1/projects/{{project}}/locations/{{location}}/preferenceSets/{{preference_set_id}}"
