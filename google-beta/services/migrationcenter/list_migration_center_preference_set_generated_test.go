...
func TestAccMigrationCenterPreferenceSetListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					listScope.Capture(map[string]string{
						"location": "google_migration_center_preference_set.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccMigrationCenterPreferenceSet_preferenceSetBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_migration_center_preference_set.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_migration_center_preference_set.list_query",
...
func testAccMigrationCenterPreferenceSet_preferenceSetBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_migration_center_preference_set" "list_query" {
    provider = google
	limit = 10000
    config {
