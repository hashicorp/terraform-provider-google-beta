...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSecurityCenterProjectCustomModule,
		DeleteFunction: testSweepSecurityCenterProjectCustomModule,
	}
...
func testSweepSecurityCenterProjectCustomModule(_ string) error {
	return listAndActionSecurityCenterProjectCustomModule(deleteResourceSecurityCenterProjectCustomModule)
}

func listAndActionSecurityCenterProjectCustomModule(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SecurityCenterProjectCustomModule"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://securitycenter.googleapis.com/v1/projects/{{project}}/securityHealthAnalyticsSettings/customModules", "?")[0]
...
func deleteResourceSecurityCenterProjectCustomModule(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://securitycenter.googleapis.com/v1/projects/{{project}}/securityHealthAnalyticsSettings/customModules/{{name}}"
