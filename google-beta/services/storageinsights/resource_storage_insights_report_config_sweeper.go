...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionStorageInsightsReportConfig,
		DeleteFunction: testSweepStorageInsightsReportConfig,
	}
...
func testSweepStorageInsightsReportConfig(_ string) error {
	return listAndActionStorageInsightsReportConfig(deleteResourceStorageInsightsReportConfig)
}

func listAndActionStorageInsightsReportConfig(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "StorageInsightsReportConfig"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://storageinsights.googleapis.com/v1/projects/{{project}}/locations/{{location}}/reportConfigs", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'reportConfigs'")
		} else {
...
func deleteResourceStorageInsightsReportConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
