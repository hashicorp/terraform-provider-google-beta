...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionStorageInsightsDatasetConfig,
		DeleteFunction: testSweepStorageInsightsDatasetConfig,
	}
...
func testSweepStorageInsightsDatasetConfig(_ string) error {
	return listAndActionStorageInsightsDatasetConfig(deleteResourceStorageInsightsDatasetConfig)
}

func listAndActionStorageInsightsDatasetConfig(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "StorageInsightsDatasetConfig"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://storageinsights.googleapis.com/v1/projects/{{project}}/locations/{{location}}/datasetConfigs", "?")[0]
		...
		resourceList, ok := res["datasetConfigs"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'datasetConfigs'")
		} else {
...
func deleteResourceStorageInsightsDatasetConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://storageinsights.googleapis.com/v1/projects/{{project}}/locations/{{location}}/datasetConfigs/{{dataset_config_id}}"
