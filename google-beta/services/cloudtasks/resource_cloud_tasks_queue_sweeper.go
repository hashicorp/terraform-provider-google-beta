...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionCloudTasksQueue,
		DeleteFunction: testSweepCloudTasksQueue,
	}
...
func testSweepCloudTasksQueue(_ string) error {
	return listAndActionCloudTasksQueue(deleteResourceCloudTasksQueue)
}

func listAndActionCloudTasksQueue(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'queues'")
		} else {
...
func deleteResourceCloudTasksQueue(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
