...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_connectivity_destination",
		ListAndAction:  listAndActionNetworkConnectivityDestination,
		DeleteFunction: testSweepNetworkConnectivityDestination,
	}
	// Add parent relationship
	s.Parents = []string{"google_network_connectivity_multicloud_data_transfer_config"}

	// Register the sweeper
	sweeper.AddTestSweepers(s)
}

func testSweepNetworkConnectivityDestination(_ string) error {
	return listAndActionNetworkConnectivityDestination(deleteResourceNetworkConnectivityDestination)
}

func listAndActionNetworkConnectivityDestination(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkConnectivityDestination"
	...
	parentType := "google_network_connectivity_multicloud_data_transfer_config"
	...
	if err := parentSweeper.ListAndAction(collectParentConfigNetworkConnectivityDestination(&configs)); err != nil {
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkconnectivity.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/multicloudDataTransferConfigs/{{multicloud_data_transfer_config}}/destinations", "?")[0]
		...
		parentValue := ""
		if v, ok := mockConfig.FieldsInSchema["multicloud_data_transfer_config"]; ok {
		...
		resourceList, ok := res["destinations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'destinations'")
		} else {
...
func deleteResourceNetworkConnectivityDestination(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkconnectivity.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/multicloudDataTransferConfigs/{{multicloud_data_transfer_config}}/destinations/{{name}}"
...
// collectParentConfigNetworkConnectivityDestination returns a function that collects parent configurations
func collectParentConfigNetworkConnectivityDestination(configs *[]*tpgresource.ResourceDataMock) sweeper.ResourceAction {
	return func(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, parentObj map[string]interface{}) error {
		...
		if parentObj["name"] == nil {
			log.Printf("[INFO][SWEEPER_LOG] Parent google_network_connectivity_multicloud_data_transfer_config field name was nil, skipping")
			return nil
		}
		...
		childConfig.FieldsInSchema["multicloud_data_transfer_config"] = parentValue
