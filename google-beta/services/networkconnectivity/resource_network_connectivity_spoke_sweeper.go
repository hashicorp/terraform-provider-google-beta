...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkConnectivitySpoke,
		DeleteFunction: testSweepNetworkConnectivitySpoke,
	}
...
func testSweepNetworkConnectivitySpoke(_ string) error {
	return listAndActionNetworkConnectivitySpoke(deleteResourceNetworkConnectivitySpoke)
}

func listAndActionNetworkConnectivitySpoke(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkconnectivity.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/spokes", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'spokes'")
		} else {
...
func deleteResourceNetworkConnectivitySpoke(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkconnectivity.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/spokes/{{name}}"
