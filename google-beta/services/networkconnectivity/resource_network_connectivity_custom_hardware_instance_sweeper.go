...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkConnectivityCustomHardwareInstance,
		DeleteFunction: testSweepNetworkConnectivityCustomHardwareInstance,
	}
...
func testSweepNetworkConnectivityCustomHardwareInstance(_ string) error {
	return listAndActionNetworkConnectivityCustomHardwareInstance(deleteResourceNetworkConnectivityCustomHardwareInstance)
}

func listAndActionNetworkConnectivityCustomHardwareInstance(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkconnectivity.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/customHardwareInstances", "?")[0]
		...
		resourceList, ok := res["customHardwareInstances"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'customHardwareInstances'")
		} else {
...
func deleteResourceNetworkConnectivityCustomHardwareInstance(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
