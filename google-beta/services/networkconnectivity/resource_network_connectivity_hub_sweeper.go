...
func testSweepNetworkConnectivityHub(_ string) error {
	return listAndActionNetworkConnectivityHub(deleteResourceNetworkConnectivityHub)
}

func listAndActionNetworkConnectivityHub(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'hubs'")
		} else {
...
func deleteResourceNetworkConnectivityHub(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
