...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionStorageTransferAgentPool,
		DeleteFunction: testSweepStorageTransferAgentPool,
	}
...
func testSweepStorageTransferAgentPool(_ string) error {
	return listAndActionStorageTransferAgentPool(deleteResourceStorageTransferAgentPool)
}

func listAndActionStorageTransferAgentPool(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'agentPools'")
		} else {
...
func deleteResourceStorageTransferAgentPool(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
