...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionLicenseManagerConfiguration,
		DeleteFunction: testSweepLicenseManagerConfiguration,
	}
...
func testSweepLicenseManagerConfiguration(_ string) error {
	return listAndActionLicenseManagerConfiguration(deleteResourceLicenseManagerConfiguration)
}

func listAndActionLicenseManagerConfiguration(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "LicenseManagerConfiguration"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://licensemanager.googleapis.com/v1alpha/projects/{{project}}/locations/{{location}}/configurations", "?")[0]
		...
		resourceList, ok := res["configurations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'configurations'")
		} else {
...
func deleteResourceLicenseManagerConfiguration(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://licensemanager.googleapis.com/v1alpha/projects/{{project}}/locations/{{location}}/configurations/{{configuration_id}}"
