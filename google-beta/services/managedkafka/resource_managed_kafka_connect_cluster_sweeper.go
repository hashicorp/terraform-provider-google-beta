...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_managed_kafka_connect_cluster",
		ListAndAction:  listAndActionManagedKafkaConnectCluster,
		DeleteFunction: testSweepManagedKafkaConnectCluster,
	}
...
func testSweepManagedKafkaConnectCluster(_ string) error {
	return listAndActionManagedKafkaConnectCluster(deleteResourceManagedKafkaConnectCluster)
}

func listAndActionManagedKafkaConnectCluster(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ManagedKafkaConnectCluster"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://managedkafka.googleapis.com/v1/projects/{{project}}/locations/{{location}}/connectClusters", "?")[0]
		...
		resourceList, ok := res["connectClusters"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'connectClusters'")
		} else {
...
func deleteResourceManagedKafkaConnectCluster(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://managedkafka.googleapis.com/v1/projects/{{project}}/locations/{{location}}/connectClusters/{{connect_cluster_id}}"
