...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionManagedKafkaCluster,
		DeleteFunction: testSweepManagedKafkaCluster,
	}
...
func testSweepManagedKafkaCluster(_ string) error {
	return listAndActionManagedKafkaCluster(deleteResourceManagedKafkaCluster)
}

func listAndActionManagedKafkaCluster(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ManagedKafkaCluster"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://managedkafka.googleapis.com/v1/projects/{{project}}/locations/{{location}}/clusters", "?")[0]
...
func deleteResourceManagedKafkaCluster(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://managedkafka.googleapis.com/v1/projects/{{project}}/locations/{{location}}/clusters/{{cluster_id}}"
