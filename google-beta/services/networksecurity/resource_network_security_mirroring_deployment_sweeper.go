...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_security_mirroring_deployment",
		ListAndAction:  listAndActionNetworkSecurityMirroringDeployment,
		DeleteFunction: testSweepNetworkSecurityMirroringDeployment,
	}
...
func testSweepNetworkSecurityMirroringDeployment(_ string) error {
	return listAndActionNetworkSecurityMirroringDeployment(deleteResourceNetworkSecurityMirroringDeployment)
}

func listAndActionNetworkSecurityMirroringDeployment(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityMirroringDeployment"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringDeployments", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'mirroringDeployments'")
		} else {
...
func deleteResourceNetworkSecurityMirroringDeployment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringDeployments/{{mirroring_deployment_id}}"
