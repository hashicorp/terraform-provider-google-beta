...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkSecurityAuthzPolicyListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityAuthzPolicyListResource{}

type NetworkSecurityAuthzPolicyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityAuthzPolicyListResource() list.ListResource {
	listR := &NetworkSecurityAuthzPolicyListResource{}
	listR.TypeName = "google_network_security_authz_policy"
	listR.SDKv2Resource = ResourceNetworkSecurityAuthzPolicy()
...
// NetworkSecurityAuthzPolicyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityAuthzPolicyListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityAuthzPolicyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityAuthzPolicyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityAuthzPolicys(
			listR.Client,
			location,
			project,
...
func ListNetworkSecurityAuthzPolicys(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityAuthzPolicy().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/{{location}}/authzPolicies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityAuthzPolicy(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "authzPolicies",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkSecurityAuthzPolicyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
