...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityInterceptDeployment,
		DeleteFunction: testSweepNetworkSecurityInterceptDeployment,
	}
...
func testSweepNetworkSecurityInterceptDeployment(_ string) error {
	return listAndActionNetworkSecurityInterceptDeployment(deleteResourceNetworkSecurityInterceptDeployment)
}

func listAndActionNetworkSecurityInterceptDeployment(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'interceptDeployments'")
		} else {
...
func deleteResourceNetworkSecurityInterceptDeployment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
