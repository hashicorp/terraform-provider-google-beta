...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityMirroringEndpoint,
		DeleteFunction: testSweepNetworkSecurityMirroringEndpoint,
	}
...
func testSweepNetworkSecurityMirroringEndpoint(_ string) error {
	return listAndActionNetworkSecurityMirroringEndpoint(deleteResourceNetworkSecurityMirroringEndpoint)
}

func listAndActionNetworkSecurityMirroringEndpoint(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityMirroringEndpoint"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringEndpoints", "?")[0]
		...
		resourceList, ok := res["mirroringEndpoints"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'mirroringEndpoints'")
		} else {
...
func deleteResourceNetworkSecurityMirroringEndpoint(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringEndpoints/{{mirroring_endpoint_id}}"
