...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityUrlLists,
		DeleteFunction: testSweepNetworkSecurityUrlLists,
	}
...
func testSweepNetworkSecurityUrlLists(_ string) error {
	return listAndActionNetworkSecurityUrlLists(deleteResourceNetworkSecurityUrlLists)
}

func listAndActionNetworkSecurityUrlLists(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityUrlLists"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/urlLists", "?")[0]
		...
		resourceList, ok := res["urlLists"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'urlLists'")
		} else {
...
func deleteResourceNetworkSecurityUrlLists(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/urlLists/{{name}}"
