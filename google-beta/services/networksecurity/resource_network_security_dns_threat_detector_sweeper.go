...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityDnsThreatDetector,
		DeleteFunction: testSweepNetworkSecurityDnsThreatDetector,
	}
...
func testSweepNetworkSecurityDnsThreatDetector(_ string) error {
	return listAndActionNetworkSecurityDnsThreatDetector(deleteResourceNetworkSecurityDnsThreatDetector)
}

func listAndActionNetworkSecurityDnsThreatDetector(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityDnsThreatDetector"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/dnsThreatDetectors", "?")[0]
		...
		resourceList, ok := res["dnsThreatDetectors"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'dnsThreatDetectors'")
		} else {
...
func deleteResourceNetworkSecurityDnsThreatDetector(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/dnsThreatDetectors/{{name}}"
