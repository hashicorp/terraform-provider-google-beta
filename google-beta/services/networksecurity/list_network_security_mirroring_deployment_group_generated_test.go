		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_network_security_mirroring_deployment_group.default",
						[]string{
							"name",
					...
					listScope.Capture(map[string]string{
						"location": "google_network_security_mirroring_deployment_group.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkSecurityMirroringDeploymentGroup_networkSecurityMirroringDeploymentGroupBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_security_mirroring_deployment_group.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_security_mirroring_deployment_group.list_query",
...
func testAccNetworkSecurityMirroringDeploymentGroup_networkSecurityMirroringDeploymentGroupBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_security_mirroring_deployment_group" "list_query" {
    provider = google
	limit = 10000
    config {
