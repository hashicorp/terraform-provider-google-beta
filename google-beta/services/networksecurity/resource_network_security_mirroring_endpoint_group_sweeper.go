...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityMirroringEndpointGroup,
		DeleteFunction: testSweepNetworkSecurityMirroringEndpointGroup,
	}
...
func testSweepNetworkSecurityMirroringEndpointGroup(_ string) error {
	return listAndActionNetworkSecurityMirroringEndpointGroup(deleteResourceNetworkSecurityMirroringEndpointGroup)
}

func listAndActionNetworkSecurityMirroringEndpointGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityMirroringEndpointGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringEndpointGroups", "?")[0]
		...
		resourceList, ok := res["mirroringEndpointGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'mirroringEndpointGroups'")
		} else {
...
func deleteResourceNetworkSecurityMirroringEndpointGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringEndpointGroups/{{mirroring_endpoint_group_id}}"
