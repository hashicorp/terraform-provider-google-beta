...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityInterceptDeploymentGroup,
		DeleteFunction: testSweepNetworkSecurityInterceptDeploymentGroup,
	}
	// Add dependencies
...
func testSweepNetworkSecurityInterceptDeploymentGroup(_ string) error {
	return listAndActionNetworkSecurityInterceptDeploymentGroup(deleteResourceNetworkSecurityInterceptDeploymentGroup)
}

func listAndActionNetworkSecurityInterceptDeploymentGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityInterceptDeploymentGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/interceptDeploymentGroups", "?")[0]
		...
		resourceList, ok := res["interceptDeploymentGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'interceptDeploymentGroups'")
		} else {
...
func deleteResourceNetworkSecurityInterceptDeploymentGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/interceptDeploymentGroups/{{intercept_deployment_group_id}}"
