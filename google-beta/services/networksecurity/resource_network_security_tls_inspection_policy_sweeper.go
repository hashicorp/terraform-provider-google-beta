...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityTlsInspectionPolicy,
		DeleteFunction: testSweepNetworkSecurityTlsInspectionPolicy,
	}
...
func testSweepNetworkSecurityTlsInspectionPolicy(_ string) error {
	return listAndActionNetworkSecurityTlsInspectionPolicy(deleteResourceNetworkSecurityTlsInspectionPolicy)
}

func listAndActionNetworkSecurityTlsInspectionPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityTlsInspectionPolicy"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["tlsInspectionPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'tlsInspectionPolicies'")
		} else {
...
func deleteResourceNetworkSecurityTlsInspectionPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/tlsInspectionPolicies/{{name}}"
