...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityAuthzPolicy,
		DeleteFunction: testSweepNetworkSecurityAuthzPolicy,
	}
...
func testSweepNetworkSecurityAuthzPolicy(_ string) error {
	return listAndActionNetworkSecurityAuthzPolicy(deleteResourceNetworkSecurityAuthzPolicy)
}

func listAndActionNetworkSecurityAuthzPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityAuthzPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/authzPolicies", "?")[0]
		...
		resourceList, ok := res["authzPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'authzPolicies'")
		} else {
...
func deleteResourceNetworkSecurityAuthzPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/authzPolicies/{{name}}"
