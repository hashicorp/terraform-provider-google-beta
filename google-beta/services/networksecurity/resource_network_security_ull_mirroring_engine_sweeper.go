...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityUllMirroringEngine,
		DeleteFunction: testSweepNetworkSecurityUllMirroringEngine,
	}
...
func testSweepNetworkSecurityUllMirroringEngine(_ string) error {
	return listAndActionNetworkSecurityUllMirroringEngine(deleteResourceNetworkSecurityUllMirroringEngine)
}

func listAndActionNetworkSecurityUllMirroringEngine(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityUllMirroringEngine"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/ullMirroringEngines", "?")[0]
		...
		resourceList, ok := res["ullMirroringEngines"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'ullMirroringEngines'")
		} else {
...
func deleteResourceNetworkSecurityUllMirroringEngine(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/ullMirroringEngines/{{ull_mirroring_engine_id}}"
