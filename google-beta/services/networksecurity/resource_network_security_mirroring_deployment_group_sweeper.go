...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityMirroringDeploymentGroup,
		DeleteFunction: testSweepNetworkSecurityMirroringDeploymentGroup,
	}
...
func testSweepNetworkSecurityMirroringDeploymentGroup(_ string) error {
	return listAndActionNetworkSecurityMirroringDeploymentGroup(deleteResourceNetworkSecurityMirroringDeploymentGroup)
}

func listAndActionNetworkSecurityMirroringDeploymentGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityMirroringDeploymentGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringDeploymentGroups", "?")[0]
		...
		resourceList, ok := res["mirroringDeploymentGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'mirroringDeploymentGroups'")
		} else {
...
func deleteResourceNetworkSecurityMirroringDeploymentGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/mirroringDeploymentGroups/{{mirroring_deployment_group_id}}"
