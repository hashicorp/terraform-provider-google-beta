		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_network_security_intercept_deployment.default",
						[]string{
							"name",
					...
					listScope.Capture(map[string]string{
						"location": "google_network_security_intercept_deployment.default",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkSecurityInterceptDeployment_networkSecurityInterceptDeploymentBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_security_intercept_deployment.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_security_intercept_deployment.list_query",
...
func testAccNetworkSecurityInterceptDeployment_networkSecurityInterceptDeploymentBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_security_intercept_deployment" "list_query" {
    provider = google
	limit = 10000
    config {
