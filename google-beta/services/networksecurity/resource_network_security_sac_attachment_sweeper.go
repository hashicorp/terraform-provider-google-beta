...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecuritySacAttachment,
		DeleteFunction: testSweepNetworkSecuritySacAttachment,
	}
...
func testSweepNetworkSecuritySacAttachment(_ string) error {
	return listAndActionNetworkSecuritySacAttachment(deleteResourceNetworkSecuritySacAttachment)
}

func listAndActionNetworkSecuritySacAttachment(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecuritySacAttachment"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/sacAttachments", "?")[0]
		...
		resourceList, ok := res["sacAttachments"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'sacAttachments'")
		} else {
...
func deleteResourceNetworkSecuritySacAttachment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/sacAttachments/{{name}}"
