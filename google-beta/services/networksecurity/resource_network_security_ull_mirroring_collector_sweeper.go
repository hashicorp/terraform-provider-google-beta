...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityUllMirroringCollector,
		DeleteFunction: testSweepNetworkSecurityUllMirroringCollector,
	}
...
func testSweepNetworkSecurityUllMirroringCollector(_ string) error {
	return listAndActionNetworkSecurityUllMirroringCollector(deleteResourceNetworkSecurityUllMirroringCollector)
}

func listAndActionNetworkSecurityUllMirroringCollector(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityUllMirroringCollector"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/ullMirroringCollectors", "?")[0]
		...
		resourceList, ok := res["ullMirroringCollectors"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'ullMirroringCollectors'")
		} else {
...
func deleteResourceNetworkSecurityUllMirroringCollector(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/ullMirroringCollectors/{{ull_mirroring_collector_id}}"
