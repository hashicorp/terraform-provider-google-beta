...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityServerTlsPolicy,
		DeleteFunction: testSweepNetworkSecurityServerTlsPolicy,
	}
...
func testSweepNetworkSecurityServerTlsPolicy(_ string) error {
	return listAndActionNetworkSecurityServerTlsPolicy(deleteResourceNetworkSecurityServerTlsPolicy)
}

func listAndActionNetworkSecurityServerTlsPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityServerTlsPolicy"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["serverTlsPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'serverTlsPolicies'")
		} else {
...
func deleteResourceNetworkSecurityServerTlsPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
