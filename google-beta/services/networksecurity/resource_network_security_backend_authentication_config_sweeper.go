...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_security_backend_authentication_config",
		ListAndAction:  listAndActionNetworkSecurityBackendAuthenticationConfig,
		DeleteFunction: testSweepNetworkSecurityBackendAuthenticationConfig,
	}
...
func testSweepNetworkSecurityBackendAuthenticationConfig(_ string) error {
	return listAndActionNetworkSecurityBackendAuthenticationConfig(deleteResourceNetworkSecurityBackendAuthenticationConfig)
}

func listAndActionNetworkSecurityBackendAuthenticationConfig(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityBackendAuthenticationConfig"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/backendAuthenticationConfigs", "?")[0]
		...
		resourceList, ok := res["backendAuthenticationConfigs"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'backendAuthenticationConfigs'")
		} else {
...
func deleteResourceNetworkSecurityBackendAuthenticationConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/backendAuthenticationConfigs/{{name}}"
