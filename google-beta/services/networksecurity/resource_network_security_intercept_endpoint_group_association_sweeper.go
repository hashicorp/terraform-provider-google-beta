...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityInterceptEndpointGroupAssociation,
		DeleteFunction: testSweepNetworkSecurityInterceptEndpointGroupAssociation,
	}
...
func testSweepNetworkSecurityInterceptEndpointGroupAssociation(_ string) error {
	return listAndActionNetworkSecurityInterceptEndpointGroupAssociation(deleteResourceNetworkSecurityInterceptEndpointGroupAssociation)
}

func listAndActionNetworkSecurityInterceptEndpointGroupAssociation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityInterceptEndpointGroupAssociation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/interceptEndpointGroupAssociations", "?")[0]
		...
		resourceList, ok := res["interceptEndpointGroupAssociations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'interceptEndpointGroupAssociations'")
		} else {
...
func deleteResourceNetworkSecurityInterceptEndpointGroupAssociation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/interceptEndpointGroupAssociations/{{intercept_endpoint_group_association_id}}"
