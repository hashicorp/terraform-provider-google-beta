...
func init() {
	registry.FrameworkListResource{
		Name:        "google_network_security_mirroring_endpoint_group",
		ProductName: "NetworkSecurity",
		Func:        NewNetworkSecurityMirroringEndpointGroupListResource,
	}.Register()
}

var _ list.ListResource = &NetworkSecurityMirroringEndpointGroupListResource{}

type NetworkSecurityMirroringEndpointGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkSecurityMirroringEndpointGroupListResource() list.ListResource {
	listR := &NetworkSecurityMirroringEndpointGroupListResource{}
	listR.TypeName = "google_network_security_mirroring_endpoint_group"
	listR.SDKv2Resource = ResourceNetworkSecurityMirroringEndpointGroup()
...
// NetworkSecurityMirroringEndpointGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkSecurityMirroringEndpointGroupListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkSecurityMirroringEndpointGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkSecurityMirroringEndpointGroupListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkSecurityMirroringEndpointGroups(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "mirroring_endpoint_group_id"); err != nil {
					return err
				}
...
func ListNetworkSecurityMirroringEndpointGroups(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkSecurityMirroringEndpointGroup().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkSecurityBasePath}}projects/{{project}}/locations/{{location}}/mirroringEndpointGroups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkSecurityMirroringEndpointGroup(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "mirroringEndpointGroups",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["mirroringEndpointGroupId"]; ok && v != nil {
				if s, ok := v.(string); ok {
					v = tpgresource.GetResourceNameFromSelfLink(s)
				}
				if err := d.Set("mirroring_endpoint_group_id", v); err != nil {
					return fmt.Errorf("error setting mirroring_endpoint_group_id: %w", err)
				}
			}
			if err = ResourceNetworkSecurityMirroringEndpointGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
