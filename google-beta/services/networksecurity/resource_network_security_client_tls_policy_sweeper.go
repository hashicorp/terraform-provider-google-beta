...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityClientTlsPolicy,
		DeleteFunction: testSweepNetworkSecurityClientTlsPolicy,
	}
...
func testSweepNetworkSecurityClientTlsPolicy(_ string) error {
	return listAndActionNetworkSecurityClientTlsPolicy(deleteResourceNetworkSecurityClientTlsPolicy)
}

func listAndActionNetworkSecurityClientTlsPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityClientTlsPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/clientTlsPolicies", "?")[0]
		...
		resourceList, ok := res["clientTlsPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'clientTlsPolicies'")
		} else {
...
func deleteResourceNetworkSecurityClientTlsPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/clientTlsPolicies/{{name}}"
