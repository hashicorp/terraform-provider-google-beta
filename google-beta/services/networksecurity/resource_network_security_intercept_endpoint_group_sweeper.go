...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecurityInterceptEndpointGroup,
		DeleteFunction: testSweepNetworkSecurityInterceptEndpointGroup,
	}
	// Add dependencies
...
func testSweepNetworkSecurityInterceptEndpointGroup(_ string) error {
	return listAndActionNetworkSecurityInterceptEndpointGroup(deleteResourceNetworkSecurityInterceptEndpointGroup)
}

func listAndActionNetworkSecurityInterceptEndpointGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecurityInterceptEndpointGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/interceptEndpointGroups", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'interceptEndpointGroups'")
		} else {
...
func deleteResourceNetworkSecurityInterceptEndpointGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/interceptEndpointGroups/{{intercept_endpoint_group_id}}"
