...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkSecuritySacRealm,
		DeleteFunction: testSweepNetworkSecuritySacRealm,
	}
...
func testSweepNetworkSecuritySacRealm(_ string) error {
	return listAndActionNetworkSecuritySacRealm(deleteResourceNetworkSecuritySacRealm)
}

func listAndActionNetworkSecuritySacRealm(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkSecuritySacRealm"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/global/sacRealms", "?")[0]
		...
		resourceList, ok := res["sacRealms"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'sacRealms'")
		} else {
...
func deleteResourceNetworkSecuritySacRealm(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networksecurity.googleapis.com/v1beta1/projects/{{project}}/locations/global/sacRealms/{{name}}"
