...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionObservabilityTraceScope,
		DeleteFunction: testSweepObservabilityTraceScope,
	}
...
func testSweepObservabilityTraceScope(_ string) error {
	return listAndActionObservabilityTraceScope(deleteResourceObservabilityTraceScope)
}

func listAndActionObservabilityTraceScope(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ObservabilityTraceScope"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://observability.googleapis.com/v1/projects/{{project}}/locations/{{location}}/traceScopes", "?")[0]
		...
		resourceList, ok := res["traceScopes"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'traceScopes'")
		} else {
...
func deleteResourceObservabilityTraceScope(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://observability.googleapis.com/v1/projects/{{project}}/locations/{{location}}/traceScopes/{{trace_scope_id}}"
