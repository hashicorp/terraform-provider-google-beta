...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataformFolder,
		DeleteFunction: testSweepDataformFolder,
	}
...
func testSweepDataformFolder(_ string) error {
	return listAndActionDataformFolder(deleteResourceDataformFolder)
}

func listAndActionDataformFolder(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DataformFolder"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://dataform.googleapis.com/v1beta1/", "?")[0]
		...
		resourceList, ok := res["folders"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'folders'")
		} else {
...
func deleteResourceDataformFolder(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://dataform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/folders/{{folder_id}}"
