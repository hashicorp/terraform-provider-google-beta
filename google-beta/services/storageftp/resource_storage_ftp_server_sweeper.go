...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionStorageFtpServer,
		DeleteFunction: testSweepStorageFtpServer,
	}
...
func testSweepStorageFtpServer(_ string) error {
	return listAndActionStorageFtpServer(deleteResourceStorageFtpServer)
}

func listAndActionStorageFtpServer(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "StorageFtpServer"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://ftp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/servers", "?")[0]
		...
		resourceList, ok := res["servers"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'servers'")
		} else {
...
func deleteResourceStorageFtpServer(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://ftp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/servers/{{server_id}}"
