...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNotebooksEnvironment,
		DeleteFunction: testSweepNotebooksEnvironment,
	}
...
func testSweepNotebooksEnvironment(_ string) error {
	return listAndActionNotebooksEnvironment(deleteResourceNotebooksEnvironment)
}

func listAndActionNotebooksEnvironment(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[0]["region"] = "us-west1-a"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'environments'")
		} else {
...
func deleteResourceNotebooksEnvironment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
