...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Pubsub",
		Func:        NewPubsubSubscriptionListResource,
	}.Register()
}

var _ list.ListResource = &PubsubSubscriptionListResource{}

type PubsubSubscriptionListResource struct {
	tpgresource.ListResourceMetadata
}

func NewPubsubSubscriptionListResource() list.ListResource {
	listR := &PubsubSubscriptionListResource{}
	listR.TypeName = "google_pubsub_subscription"
	listR.SDKv2Resource = ResourcePubsubSubscription()
...
// PubsubSubscriptionListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type PubsubSubscriptionListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *PubsubSubscriptionListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data PubsubSubscriptionListModel
...
func ListPubsubSubscriptions(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourcePubsubSubscription().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{PubsubBasePath}}projects/{{project}}/subscriptions")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourcePubsubSubscriptionFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
