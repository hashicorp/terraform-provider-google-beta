...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPubsubSubscription,
		DeleteFunction: testSweepPubsubSubscription,
	}
...
func testSweepPubsubSubscription(_ string) error {
	return listAndActionPubsubSubscription(deleteResourcePubsubSubscription)
}

func listAndActionPubsubSubscription(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://pubsub.googleapis.com/v1/projects/{{project}}/subscriptions", "?")[0]
...
func deleteResourcePubsubSubscription(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://pubsub.googleapis.com/v1/projects/{{project}}/subscriptions/{{name}}"
