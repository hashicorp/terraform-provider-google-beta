...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPubsubTopic,
		DeleteFunction: testSweepPubsubTopic,
	}
...
func testSweepPubsubTopic(_ string) error {
	return listAndActionPubsubTopic(deleteResourcePubsubTopic)
}

func listAndActionPubsubTopic(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "PubsubTopic"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://pubsub.googleapis.com/v1/projects/{{project}}/topics", "?")[0]
...
func deleteResourcePubsubTopic(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://pubsub.googleapis.com/v1/projects/{{project}}/topics/{{name}}"
