...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPubsubSchema,
		DeleteFunction: testSweepPubsubSchema,
	}
...
func testSweepPubsubSchema(_ string) error {
	return listAndActionPubsubSchema(deleteResourcePubsubSchema)
}

func listAndActionPubsubSchema(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "PubsubSchema"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://pubsub.googleapis.com/v1/projects/{{project}}/schemas", "?")[0]
...
func deleteResourcePubsubSchema(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://pubsub.googleapis.com/v1/projects/{{project}}/schemas/{{name}}"
