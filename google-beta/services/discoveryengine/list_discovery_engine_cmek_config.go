...
func init() {
	registry.FrameworkListResource{
		Name:        "google_discovery_engine_cmek_config",
		ProductName: "DiscoveryEngine",
		Func:        NewDiscoveryEngineCmekConfigListResource,
	}.Register()
}

var _ list.ListResource = &DiscoveryEngineCmekConfigListResource{}

type DiscoveryEngineCmekConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewDiscoveryEngineCmekConfigListResource() list.ListResource {
	listR := &DiscoveryEngineCmekConfigListResource{}
	listR.TypeName = "google_discovery_engine_cmek_config"
	listR.SDKv2Resource = ResourceDiscoveryEngineCmekConfig()
...
// DiscoveryEngineCmekConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type DiscoveryEngineCmekConfigListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *DiscoveryEngineCmekConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data DiscoveryEngineCmekConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListDiscoveryEngineCmekConfigs(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "cmek_config_id"); err != nil {
					return err
				}
...
func ListDiscoveryEngineCmekConfigs(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceDiscoveryEngineCmekConfig().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{DiscoveryEngineBasePath}}projects/{{project}}/locations/{{location}}/cmekConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceDiscoveryEngineCmekConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "cmekConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["cmekConfigId"]; ok && v != nil {
				if err := d.Set("cmek_config_id", v); err != nil {
					return fmt.Errorf("error setting cmek_config_id: %w", err)
				}
			}
			if err = ResourceDiscoveryEngineCmekConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
