...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDiscoveryEngineCmekConfig,
		DeleteFunction: testSweepDiscoveryEngineCmekConfig,
	}
...
func testSweepDiscoveryEngineCmekConfig(_ string) error {
	return listAndActionDiscoveryEngineCmekConfig(deleteResourceDiscoveryEngineCmekConfig)
}

func listAndActionDiscoveryEngineCmekConfig(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DiscoveryEngineCmekConfig"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{location}}-discoveryengine.googleapis.com/v1/projects/{{project}}/locations/{{location}}/cmekConfigs", "?")[0]
		...
		resourceList, ok := res["cmekConfigs"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'cmekConfigs'")
		} else {
...
func deleteResourceDiscoveryEngineCmekConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{location}}-discoveryengine.googleapis.com/v1/projects/{{project}}/locations/{{location}}/cmekConfigs/{{cmek_config_id}}"
