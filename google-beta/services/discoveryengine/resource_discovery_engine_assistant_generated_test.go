...
func TestAccDiscoveryEngineAssistant_discoveryengineAssistantBasicExample(t *testing.T) {
	...
	context := map[string]interface{}{
		"data_store_id": "tf-test-example-data-store-id" + randomSuffix,
		"engine_id":     "tf-test-example-engine-id" + randomSuffix,
		"template_id":   "tf-test-example-template-id" + randomSuffix,
...
func testAccCheckDiscoveryEngineAssistantDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(discoveryengine.Product, config), "projects/{{project}}/locations/{{location}}/collections/{{collection_id}}/engines/{{engine_id}}/assistants/{{assistant_id}}"))
			if err != nil {
				return err
			}
