...
func init() {
	registry.FrameworkListResource{
		Name:        "google_discovery_engine_license_config",
		ProductName: "DiscoveryEngine",
		Func:        NewDiscoveryEngineLicenseConfigListResource,
	}.Register()
}

var _ list.ListResource = &DiscoveryEngineLicenseConfigListResource{}

type DiscoveryEngineLicenseConfigListResource struct {
	tpgresource.ListResourceMetadata
}

func NewDiscoveryEngineLicenseConfigListResource() list.ListResource {
	listR := &DiscoveryEngineLicenseConfigListResource{}
	listR.TypeName = "google_discovery_engine_license_config"
	listR.SDKv2Resource = ResourceDiscoveryEngineLicenseConfig()
...
// DiscoveryEngineLicenseConfigListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type DiscoveryEngineLicenseConfigListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *DiscoveryEngineLicenseConfigListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data DiscoveryEngineLicenseConfigListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListDiscoveryEngineLicenseConfigs(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "license_config_id"); err != nil {
					return err
				}
...
func ListDiscoveryEngineLicenseConfigs(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceDiscoveryEngineLicenseConfig().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{DiscoveryEngineBasePath}}projects/{{project}}/locations/{{location}}/licenseConfigs")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceDiscoveryEngineLicenseConfig(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "licenseConfigs",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["licenseConfigId"]; ok && v != nil {
				if err := d.Set("license_config_id", v); err != nil {
					return fmt.Errorf("error setting license_config_id: %w", err)
				}
			}
			if err = ResourceDiscoveryEngineLicenseConfigFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
