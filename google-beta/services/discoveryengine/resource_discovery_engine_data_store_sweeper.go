...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDiscoveryEngineDataStore,
		DeleteFunction: testSweepDiscoveryEngineDataStore,
	}
	// Add dependencies
...
func testSweepDiscoveryEngineDataStore(_ string) error {
	return listAndActionDiscoveryEngineDataStore(deleteResourceDiscoveryEngineDataStore)
}

func listAndActionDiscoveryEngineDataStore(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DiscoveryEngineDataStore"
	...
	intermediateValues[0]["region"] = "eu"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{location}}-discoveryengine.googleapis.com/v1/projects/{{project}}/locations/{{location}}/collections/default_collection/dataStores", "?")[0]
...
func deleteResourceDiscoveryEngineDataStore(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{location}}-discoveryengine.googleapis.com/v1/projects/{{project}}/locations/{{location}}/collections/default_collection/dataStores/{{data_store_id}}"
