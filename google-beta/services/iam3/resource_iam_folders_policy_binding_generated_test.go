...
func testAccCheckIAM3FoldersPolicyBindingDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(iam3.Product, config), "folders/{{folder}}/locations/{{location}}/policyBindings/{{policy_binding_id}}"))
			if err != nil {
				return err
			}
