...
func init() {
	registry.FrameworkListResource{
		Name:        "google_gke_backup_backup_channel",
		ProductName: "GKEBackup",
		Func:        NewGKEBackupBackupChannelListResource,
	}.Register()
}

var _ list.ListResource = &GKEBackupBackupChannelListResource{}

type GKEBackupBackupChannelListResource struct {
	tpgresource.ListResourceMetadata
}

func NewGKEBackupBackupChannelListResource() list.ListResource {
	listR := &GKEBackupBackupChannelListResource{}
	listR.TypeName = "google_gke_backup_backup_channel"
	listR.SDKv2Resource = ResourceGKEBackupBackupChannel()
...
// GKEBackupBackupChannelListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type GKEBackupBackupChannelListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *GKEBackupBackupChannelListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data GKEBackupBackupChannelListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListGKEBackupBackupChannels(
			listR.Client,
			location,
			project,
...
func ListGKEBackupBackupChannels(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceGKEBackupBackupChannel().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{GKEBackupBasePath}}projects/{{project}}/locations/{{location}}/backupChannels")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceGKEBackupBackupChannel(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "backupChannels",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceGKEBackupBackupChannelFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
			if location != "" {
