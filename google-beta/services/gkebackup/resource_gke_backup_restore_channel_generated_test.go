...
func testAccCheckGKEBackupRestoreChannelDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_gke_backup_restore_channel" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, "{{GKEBackupBasePath}}projects/{{project}}/locations/{{location}}/restoreChannels/{{name}}")
			if err != nil {
				return err
			}
			...
			if err == nil {
				return fmt.Errorf("GKEBackupRestoreChannel still exists at %s", url)
			}
		}
