...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionGKEBackupRestorePlan,
		DeleteFunction: testSweepGKEBackupRestorePlan,
	}
...
func testSweepGKEBackupRestorePlan(_ string) error {
	return listAndActionGKEBackupRestorePlan(deleteResourceGKEBackupRestorePlan)
}

func listAndActionGKEBackupRestorePlan(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GKEBackupRestorePlan"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://gkebackup.googleapis.com/v1/projects/{{project}}/locations/{{location}}/restorePlans", "?")[0]
		...
		resourceList, ok := res["restorePlans"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'restorePlans'")
		} else {
...
func deleteResourceGKEBackupRestorePlan(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://gkebackup.googleapis.com/v1/projects/{{project}}/locations/{{location}}/restorePlans/{{name}}"
