...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionGKEBackupBackupChannel,
		DeleteFunction: testSweepGKEBackupBackupChannel,
	}
...
func testSweepGKEBackupBackupChannel(_ string) error {
	return listAndActionGKEBackupBackupChannel(deleteResourceGKEBackupBackupChannel)
}

func listAndActionGKEBackupBackupChannel(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GKEBackupBackupChannel"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://gkebackup.googleapis.com/v1/projects/{{project}}/locations/{{location}}/backupChannels", "?")[0]
		...
		resourceList, ok := res["backupChannels"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'backupChannels'")
		} else {
...
func deleteResourceGKEBackupBackupChannel(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://gkebackup.googleapis.com/v1/projects/{{project}}/locations/{{location}}/backupChannels/{{name}}"
