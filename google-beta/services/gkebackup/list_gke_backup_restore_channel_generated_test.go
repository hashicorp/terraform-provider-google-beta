...
func TestAccGKEBackupRestoreChannelListQuery_generated(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccGKEBackupRestoreChannel_gkebackupRestorechannelBasicExample(context),
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_gke_backup_restore_channel.basic",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_gke_backup_restore_channel.basic",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccGKEBackupRestoreChannel_gkebackupRestorechannelBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_gke_backup_restore_channel.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_gke_backup_restore_channel.list_query",
...
func testAccGKEBackupRestoreChannel_gkebackupRestorechannelBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_gke_backup_restore_channel" "list_query" {
    provider = google
	limit = 10000
    config {
