...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_gke_backup_restore_channel",
		ListAndAction:  listAndActionGKEBackupRestoreChannel,
		DeleteFunction: testSweepGKEBackupRestoreChannel,
	}
...
func testSweepGKEBackupRestoreChannel(_ string) error {
	return listAndActionGKEBackupRestoreChannel(deleteResourceGKEBackupRestoreChannel)
}

func listAndActionGKEBackupRestoreChannel(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GKEBackupRestoreChannel"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://gkebackup.googleapis.com/v1/projects/{{project}}/locations/{{location}}/restoreChannels", "?")[0]
		...
		resourceList, ok := res["restoreChannels"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'restoreChannels'")
		} else {
...
func deleteResourceGKEBackupRestoreChannel(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://gkebackup.googleapis.com/v1/projects/{{project}}/locations/{{location}}/restoreChannels/{{name}}"
