...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDeveloperConnectConnection,
		DeleteFunction: testSweepDeveloperConnectConnection,
	}
...
func testSweepDeveloperConnectConnection(_ string) error {
	return listAndActionDeveloperConnectConnection(deleteResourceDeveloperConnectConnection)
}

func listAndActionDeveloperConnectConnection(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DeveloperConnectConnection"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://developerconnect.googleapis.com/v1/projects/{{project}}/locations/{{location}}/connections", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'connections'")
		} else {
...
func deleteResourceDeveloperConnectConnection(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://developerconnect.googleapis.com/v1/projects/{{project}}/locations/{{location}}/connections/{{connection_id}}"
