...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDeveloperConnectAccountConnector,
		DeleteFunction: testSweepDeveloperConnectAccountConnector,
	}
...
func testSweepDeveloperConnectAccountConnector(_ string) error {
	return listAndActionDeveloperConnectAccountConnector(deleteResourceDeveloperConnectAccountConnector)
}

func listAndActionDeveloperConnectAccountConnector(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DeveloperConnectAccountConnector"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://developerconnect.googleapis.com/v1/projects/{{project}}/locations/{{location}}/accountConnectors", "?")[0]
		...
		resourceList, ok := res["accountConnectors"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'accountConnectors'")
		} else {
...
func deleteResourceDeveloperConnectAccountConnector(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://developerconnect.googleapis.com/v1/projects/{{project}}/locations/{{location}}/accountConnectors/{{account_connector_id}}"
