...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewIdentityPlatformTenantListResource,
	}.Register()
}

var _ list.ListResource = &IdentityPlatformTenantListResource{}

type IdentityPlatformTenantListResource struct {
	tpgresource.ListResourceMetadata
}

func NewIdentityPlatformTenantListResource() list.ListResource {
	listR := &IdentityPlatformTenantListResource{}
	listR.TypeName = "google_identity_platform_tenant"
	listR.SDKv2Resource = ResourceIdentityPlatformTenant()
...
// IdentityPlatformTenantListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type IdentityPlatformTenantListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *IdentityPlatformTenantListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data IdentityPlatformTenantListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListIdentityPlatformTenants(
			listR.Client,
			project,
...
func ListIdentityPlatformTenants(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceIdentityPlatformTenant().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{IdentityPlatformBasePath}}projects/{{project}}/tenants")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceIdentityPlatformTenant(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceIdentityPlatformTenantFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
