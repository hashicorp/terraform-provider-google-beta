...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDatastreamStream,
		DeleteFunction: testSweepDatastreamStream,
	}
...
func testSweepDatastreamStream(_ string) error {
	return listAndActionDatastreamStream(deleteResourceDatastreamStream)
}

func listAndActionDatastreamStream(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'streams'")
		} else {
...
func deleteResourceDatastreamStream(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
