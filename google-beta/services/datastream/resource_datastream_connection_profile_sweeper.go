...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDatastreamConnectionProfile,
		DeleteFunction: testSweepDatastreamConnectionProfile,
	}
...
func testSweepDatastreamConnectionProfile(_ string) error {
	return listAndActionDatastreamConnectionProfile(deleteResourceDatastreamConnectionProfile)
}

func listAndActionDatastreamConnectionProfile(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DatastreamConnectionProfile"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://datastream.googleapis.com/v1/projects/{{project}}/locations/{{location}}/connectionProfiles", "?")[0]
		...
		resourceList, ok := res["connectionProfiles"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'connectionProfiles'")
		} else {
...
func deleteResourceDatastreamConnectionProfile(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://datastream.googleapis.com/v1/projects/{{project}}/locations/{{location}}/connectionProfiles/{{connection_profile_id}}"
