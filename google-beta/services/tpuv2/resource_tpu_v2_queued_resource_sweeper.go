...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_tpu_v2_queued_resource",
		ListAndAction:  listAndActionTpuV2QueuedResource,
		DeleteFunction: testSweepTpuV2QueuedResource,
	}
...
func testSweepTpuV2QueuedResource(_ string) error {
	return listAndActionTpuV2QueuedResource(deleteResourceTpuV2QueuedResource)
}

func listAndActionTpuV2QueuedResource(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "TpuV2QueuedResource"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'queuedResources'")
		} else {
...
func deleteResourceTpuV2QueuedResource(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
