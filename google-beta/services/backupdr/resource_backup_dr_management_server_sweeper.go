...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBackupDRManagementServer,
		DeleteFunction: testSweepBackupDRManagementServer,
	}
...
func testSweepBackupDRManagementServer(_ string) error {
	return listAndActionBackupDRManagementServer(deleteResourceBackupDRManagementServer)
}

func listAndActionBackupDRManagementServer(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'managementServers'")
		} else {
...
func deleteResourceBackupDRManagementServer(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
