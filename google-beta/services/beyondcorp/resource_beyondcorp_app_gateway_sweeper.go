...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBeyondcorpAppGateway,
		DeleteFunction: testSweepBeyondcorpAppGateway,
	}
...
func testSweepBeyondcorpAppGateway(_ string) error {
	return listAndActionBeyondcorpAppGateway(deleteResourceBeyondcorpAppGateway)
}

func listAndActionBeyondcorpAppGateway(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BeyondcorpAppGateway"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://beyondcorp.googleapis.com/v1/projects/{{project}}/locations/{{region}}/appGateways", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'appGateways'")
		} else {
...
func deleteResourceBeyondcorpAppGateway(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://beyondcorp.googleapis.com/v1/projects/{{project}}/locations/{{region}}/appGateways/{{name}}"
