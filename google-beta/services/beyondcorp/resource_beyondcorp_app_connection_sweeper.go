...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBeyondcorpAppConnection,
		DeleteFunction: testSweepBeyondcorpAppConnection,
	}
...
func testSweepBeyondcorpAppConnection(_ string) error {
	return listAndActionBeyondcorpAppConnection(deleteResourceBeyondcorpAppConnection)
}

func listAndActionBeyondcorpAppConnection(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'appConnections'")
		} else {
...
func deleteResourceBeyondcorpAppConnection(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
