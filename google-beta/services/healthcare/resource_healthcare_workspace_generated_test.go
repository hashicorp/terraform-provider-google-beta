...
func TestAccHealthcareWorkspace_healthcareWorkspaceBasicExample(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccHealthcareWorkspace_healthcareWorkspaceBasicExample(context),
			},
			{
...
func testAccCheckHealthcareWorkspaceDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_healthcare_workspace" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(healthcare.Product, config), "{{dataset}}/dataMapperWorkspaces/{{name}}"))
			if err != nil {
				return err
			}
			...
			if err == nil {
				return fmt.Errorf("HealthcareWorkspace still exists at %s", url)
			}
		}
