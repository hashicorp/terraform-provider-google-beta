...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSecurityScannerScanConfig,
		DeleteFunction: testSweepSecurityScannerScanConfig,
	}
...
func testSweepSecurityScannerScanConfig(_ string) error {
	return listAndActionSecurityScannerScanConfig(deleteResourceSecurityScannerScanConfig)
}

func listAndActionSecurityScannerScanConfig(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'scanConfigs'")
		} else {
...
func deleteResourceSecurityScannerScanConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
