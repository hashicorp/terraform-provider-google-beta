...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBiglakeCatalog,
		DeleteFunction: testSweepBiglakeCatalog,
	}
...
func testSweepBiglakeCatalog(_ string) error {
	return listAndActionBiglakeCatalog(deleteResourceBiglakeCatalog)
}

func listAndActionBiglakeCatalog(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BiglakeCatalog"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://biglake.googleapis.com/v1/projects/{{project}}/locations/{{location}}/catalogs", "?")[0]
...
func deleteResourceBiglakeCatalog(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://biglake.googleapis.com/v1/projects/{{project}}/locations/{{location}}/catalogs/{{name}}"
