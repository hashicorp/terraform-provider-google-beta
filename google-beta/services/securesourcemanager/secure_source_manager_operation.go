...
func (w *SecureSourceManagerOperationWaiter) QueryOp() (interface{}, error) {
	...
	url := fmt.Sprintf("%s%s", w.Config.SecureSourceManagerBasePath, w.CommonOperationWaiter.Op.Name)
...
func createSecureSourceManagerWaiter(config *transport_tpg.Config, op map[string]interface{}, project, activity, userAgent string) (*SecureSourceManagerOperationWaiter, error) {
	w := &SecureSourceManagerOperationWaiter{
		Config:    config,
		UserAgent: userAgent,
		Project:   project,
	}
...
func SecureSourceManagerOperationWaitTimeWithResponse(config *transport_tpg.Config, op map[string]interface{}, response *map[string]interface{}, project, activity, userAgent string, timeout time.Duration) error {
	w, err := createSecureSourceManagerWaiter(config, op, project, activity, userAgent)
	if err != nil {
		return err
	}
