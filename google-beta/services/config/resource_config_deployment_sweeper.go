...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionConfigDeployment,
		DeleteFunction: testSweepConfigDeployment,
	}
...
func testSweepConfigDeployment(_ string) error {
	return listAndActionConfigDeployment(deleteResourceConfigDeployment)
}

func listAndActionConfigDeployment(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ConfigDeployment"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://config.googleapis.com/v1/projects/{{project}}/locations/{{location}}/deployments", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'deployments'")
		} else {
...
func deleteResourceConfigDeployment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://config.googleapis.com/v1/projects/{{project}}/locations/{{location}}/deployments/{{name}}?force={{force_destroy}}"
