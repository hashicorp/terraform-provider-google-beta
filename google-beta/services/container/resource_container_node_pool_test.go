...
func TestAccContainerNodePool_withOpportunisticMaintenance(t *testing.T) {
	t.Parallel()
	t.Skip("Opportunistic maintenance strategy is only supported on GPU/TPU node pools; requires GPU capacity/quota and a project allowlisted for GKE host maintenance. Run manually.")
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccContainerNodePool_withOpportunisticMaintenance(cluster, networkName, subnetworkName, np),
			},
			{
				ResourceName:      "google_container_node_pool.np",
				ImportState:       true,
...
func testAccContainerNodePool_withOpportunisticMaintenance(cluster, networkName, subnetworkName, np string) string {
	return fmt.Sprintf(`
resource "google_compute_network" "net" {
  name                    = "%s"
  auto_create_subnetworks = false
  ...
  node_config {
    ...
    host_maintenance_policy {
      ...
      opportunistic_maintenance_strategy {
        node_idle_time_window           = "600s"
        maintenance_availability_window = "1209600s"
