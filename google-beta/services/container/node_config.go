...
func expandHostMaintenancePolicy(v interface{}) *container.HostMaintenancePolicy {
	...
	if v, ok := cfg["opportunistic_maintenance_strategy"]; ok {
		mPolicy.OpportunisticMaintenanceStrategy = expandOpportunisticMaintenanceStrategy(v)
	}

	return mPolicy
}

func expandOpportunisticMaintenanceStrategy(v interface{}) *container.OpportunisticMaintenanceStrategy {
	if v == nil {
		return nil
	}
	ls := v.([]interface{})
	...
	cfg := ls[0].(map[string]interface{})
	strategy := &container.OpportunisticMaintenanceStrategy{
		ForceSendFields: []string{"MinNodesPerPool"},
	}
	if nodeIdleTimeWindow, ok := cfg["node_idle_time_window"]; ok {
		strategy.NodeIdleTimeWindow = nodeIdleTimeWindow.(string)
	}
	if maintenanceAvailabilityWindow, ok := cfg["maintenance_availability_window"]; ok {
		strategy.MaintenanceAvailabilityWindow = maintenanceAvailabilityWindow.(string)
	}
	if minNodesPerPool, ok := cfg["min_nodes_per_pool"]; ok {
		strategy.MinNodesPerPool = int64(minNodesPerPool.(int))
	}
...
func flattenHostMaintenancePolicy(c *container.HostMaintenancePolicy) []map[string]interface{} {
	...
	if c != nil {
		result = append(result, map[string]interface{}{
			...
			"opportunistic_maintenance_strategy": flattenOpportunisticMaintenanceStrategy(c.OpportunisticMaintenanceStrategy),
		})
	}
...
func flattenOpportunisticMaintenanceStrategy(c *container.OpportunisticMaintenanceStrategy) []map[string]interface{} {
	result := []map[string]interface{}{}
	if c != nil {
		result = append(result, map[string]interface{}{
			"node_idle_time_window":           c.NodeIdleTimeWindow,
			"maintenance_availability_window": c.MaintenanceAvailabilityWindow,
			"min_nodes_per_pool":              c.MinNodesPerPool,
		})
	}
