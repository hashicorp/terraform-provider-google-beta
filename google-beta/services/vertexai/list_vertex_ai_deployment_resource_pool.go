...
func init() {
	registry.FrameworkListResource{
		Name:        "google_vertex_ai_deployment_resource_pool",
		ProductName: "VertexAI",
		Func:        NewVertexAIDeploymentResourcePoolListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIDeploymentResourcePoolListResource{}

type VertexAIDeploymentResourcePoolListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIDeploymentResourcePoolListResource() list.ListResource {
	listR := &VertexAIDeploymentResourcePoolListResource{}
	listR.TypeName = "google_vertex_ai_deployment_resource_pool"
	listR.SDKv2Resource = ResourceVertexAIDeploymentResourcePool()
...
// VertexAIDeploymentResourcePoolListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIDeploymentResourcePoolListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIDeploymentResourcePoolListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIDeploymentResourcePoolListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListVertexAIDeploymentResourcePools(
			listR.Client,
			region,
			project,
...
func ListVertexAIDeploymentResourcePools(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIDeploymentResourcePool().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/deploymentResourcePools")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIDeploymentResourcePool(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "deploymentResourcePools",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIDeploymentResourcePoolFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
