...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIDeploymentResourcePool,
		DeleteFunction: testSweepVertexAIDeploymentResourcePool,
	}
...
func testSweepVertexAIDeploymentResourcePool(_ string) error {
	return listAndActionVertexAIDeploymentResourcePool(deleteResourceVertexAIDeploymentResourcePool)
}

func listAndActionVertexAIDeploymentResourcePool(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIDeploymentResourcePool"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/deploymentResourcePools", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'deploymentResourcePools'")
		} else {
...
func deleteResourceVertexAIDeploymentResourcePool(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/deploymentResourcePools/{{name}}"
