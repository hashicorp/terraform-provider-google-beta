...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIOnlineEvaluator,
		DeleteFunction: testSweepVertexAIOnlineEvaluator,
	}
...
func testSweepVertexAIOnlineEvaluator(_ string) error {
	return listAndActionVertexAIOnlineEvaluator(deleteResourceVertexAIOnlineEvaluator)
}

func listAndActionVertexAIOnlineEvaluator(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIOnlineEvaluator"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/onlineEvaluators", "?")[0]
		...
		resourceList, ok := res["onlineEvaluators"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'onlineEvaluators'")
		} else {
...
func deleteResourceVertexAIOnlineEvaluator(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/onlineEvaluators/{{name}}"
