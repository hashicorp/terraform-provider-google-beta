...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIRagCorpus,
		DeleteFunction: testSweepVertexAIRagCorpus,
	}
...
func testSweepVertexAIRagCorpus(_ string) error {
	return listAndActionVertexAIRagCorpus(deleteResourceVertexAIRagCorpus)
}

func listAndActionVertexAIRagCorpus(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIRagCorpus"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/ragCorpora", "?")[0]
		...
		resourceList, ok := res["ragCorpora"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'ragCorpora'")
		} else {
...
func deleteResourceVertexAIRagCorpus(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/ragCorpora/{{name}}"
