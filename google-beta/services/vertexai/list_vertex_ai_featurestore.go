...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIFeaturestoreListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIFeaturestoreListResource{}

type VertexAIFeaturestoreListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIFeaturestoreListResource() list.ListResource {
	listR := &VertexAIFeaturestoreListResource{}
	listR.TypeName = "google_vertex_ai_featurestore"
	listR.SDKv2Resource = ResourceVertexAIFeaturestore()
...
// VertexAIFeaturestoreListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIFeaturestoreListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIFeaturestoreListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIFeaturestoreListModel
...
func ListVertexAIFeaturestores(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIFeaturestore().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/featurestores")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIFeaturestore(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "featurestores",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIFeaturestoreFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
