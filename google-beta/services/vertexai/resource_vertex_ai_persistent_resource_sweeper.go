...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIPersistentResource,
		DeleteFunction: testSweepVertexAIPersistentResource,
	}
...
func testSweepVertexAIPersistentResource(_ string) error {
	return listAndActionVertexAIPersistentResource(deleteResourceVertexAIPersistentResource)
}

func listAndActionVertexAIPersistentResource(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIPersistentResource"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/persistentResources", "?")[0]
		...
		resourceList, ok := res["persistentResources"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'persistentResources'")
		} else {
...
func deleteResourceVertexAIPersistentResource(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/persistentResources/{{name}}"
