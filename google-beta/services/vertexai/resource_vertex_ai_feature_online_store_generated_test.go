...
func TestAccVertexAIFeatureOnlineStore_vertexAiFeatureonlinestoreWithBetaFieldsBigtableExample(t *testing.T) {
	...
	context := map[string]interface{}{
		"name":          "tf_test_example_feature_online_store_beta_bigtable" + randomSuffix,
		"random_suffix": randomSuffix,
	}
...
func testAccCheckVertexAIFeatureOnlineStoreDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(vertexai.Product, config), "projects/{{project}}/locations/{{region}}/featureOnlineStores/{{name}}"))
			if err != nil {
				return err
			}
