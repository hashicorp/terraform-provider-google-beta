...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIMetadataStore,
		DeleteFunction: testSweepVertexAIMetadataStore,
	}
...
func testSweepVertexAIMetadataStore(_ string) error {
	return listAndActionVertexAIMetadataStore(deleteResourceVertexAIMetadataStore)
}

func listAndActionVertexAIMetadataStore(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIMetadataStore"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/metadataStores", "?")[0]
		...
		resourceList, ok := res["metadataStores"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'metadataStores'")
		} else {
...
func deleteResourceVertexAIMetadataStore(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/metadataStores/{{name}}"
