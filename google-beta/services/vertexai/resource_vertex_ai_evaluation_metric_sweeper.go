...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIEvaluationMetric,
		DeleteFunction: testSweepVertexAIEvaluationMetric,
	}
...
func testSweepVertexAIEvaluationMetric(_ string) error {
	return listAndActionVertexAIEvaluationMetric(deleteResourceVertexAIEvaluationMetric)
}

func listAndActionVertexAIEvaluationMetric(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIEvaluationMetric"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/evaluationMetrics", "?")[0]
		...
		resourceList, ok := res["evaluationMetrics"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'evaluationMetrics'")
		} else {
...
func deleteResourceVertexAIEvaluationMetric(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/evaluationMetrics/{{name}}"
