...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewVertexAIDatasetListResource,
	}.Register()
}

var _ list.ListResource = &VertexAIDatasetListResource{}

type VertexAIDatasetListResource struct {
	tpgresource.ListResourceMetadata
}

func NewVertexAIDatasetListResource() list.ListResource {
	listR := &VertexAIDatasetListResource{}
	listR.TypeName = "google_vertex_ai_dataset"
	listR.SDKv2Resource = ResourceVertexAIDataset()
...
// VertexAIDatasetListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type VertexAIDatasetListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *VertexAIDatasetListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data VertexAIDatasetListModel
...
func ListVertexAIDatasets(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceVertexAIDataset().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{VertexAIBasePath}}projects/{{project}}/locations/{{region}}/datasets")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceVertexAIDataset(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceVertexAIDatasetFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
