	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateIdFunc: generateVertexAIFeatureGroupIAMBindingStateID("google_vertex_ai_feature_group_iam_binding.foo"),
				ImportState:       true,
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateIdFunc: generateVertexAIFeatureGroupIAMMemberStateID("google_vertex_ai_feature_group_iam_member.foo"),
				ImportState:       true,
				ImportStateVerify: true,
			},
		},
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateIdFunc: generateVertexAIFeatureGroupIAMPolicyStateID("google_vertex_ai_feature_group_iam_policy.foo"),
				ImportState:       true,
				ImportStateVerify: true,
			},
			{
...
func generateVertexAIFeatureGroupIAMPolicyStateID(iamResourceAddr string) func(*terraform.State) (string, error) {
	return func(state *terraform.State) (string, error) {
		...
		feature_group := tpgresource.GetResourceNameFromSelfLink(rawState["feature_group"])
		return acctest.BuildIAMImportId(fmt.Sprintf("projects/%s/locations/%s/featureGroups/%s", project, region, feature_group), "", "", rawState["condition.0.title"]), nil
	}
}

func generateVertexAIFeatureGroupIAMBindingStateID(iamResourceAddr string) func(*terraform.State) (string, error) {
	return func(state *terraform.State) (string, error) {
		var rawState map[string]string
		for _, m := range state.Modules {
			if len(m.Resources) > 0 {
		...
		region := tpgresource.GetResourceNameFromSelfLink(rawState["region"])
		feature_group := tpgresource.GetResourceNameFromSelfLink(rawState["feature_group"])
		return acctest.BuildIAMImportId(fmt.Sprintf("projects/%s/locations/%s/featureGroups/%s", project, region, feature_group), rawState["role"], "", rawState["condition.0.title"]), nil
	}
}

func generateVertexAIFeatureGroupIAMMemberStateID(iamResourceAddr string) func(*terraform.State) (string, error) {
	return func(state *terraform.State) (string, error) {
		var rawState map[string]string
		for _, m := range state.Modules {
			if len(m.Resources) > 0 {
		...
		region := tpgresource.GetResourceNameFromSelfLink(rawState["region"])
		feature_group := tpgresource.GetResourceNameFromSelfLink(rawState["feature_group"])
		return acctest.BuildIAMImportId(fmt.Sprintf("projects/%s/locations/%s/featureGroups/%s", project, region, feature_group), rawState["role"], rawState["member"], rawState["condition.0.title"]), nil
	}
}
