...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAIDataset,
		DeleteFunction: testSweepVertexAIDataset,
	}
...
func testSweepVertexAIDataset(_ string) error {
	return listAndActionVertexAIDataset(deleteResourceVertexAIDataset)
}

func listAndActionVertexAIDataset(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAIDataset"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/datasets", "?")[0]
...
func deleteResourceVertexAIDataset(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
