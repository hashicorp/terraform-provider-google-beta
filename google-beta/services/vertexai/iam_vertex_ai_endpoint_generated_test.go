	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateIdFunc: generateVertexAIEndpointIAMBindingStateID("google_vertex_ai_endpoint_iam_binding.foo"),
				ImportState:       true,
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateIdFunc: generateVertexAIEndpointIAMMemberStateID("google_vertex_ai_endpoint_iam_member.foo"),
				ImportState:       true,
				ImportStateVerify: true,
			},
		},
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateIdFunc: generateVertexAIEndpointIAMPolicyStateID("google_vertex_ai_endpoint_iam_policy.foo"),
				ImportState:       true,
				ImportStateVerify: true,
			},
			{
...
func generateVertexAIEndpointIAMPolicyStateID(iamResourceAddr string) func(*terraform.State) (string, error) {
	return func(state *terraform.State) (string, error) {
		...
		endpoint := tpgresource.GetResourceNameFromSelfLink(rawState["endpoint"])
		return acctest.BuildIAMImportId(fmt.Sprintf("projects/%s/locations/%s/endpoints/%s", project, location, endpoint), "", "", rawState["condition.0.title"]), nil
	}
}

func generateVertexAIEndpointIAMBindingStateID(iamResourceAddr string) func(*terraform.State) (string, error) {
	return func(state *terraform.State) (string, error) {
		var rawState map[string]string
		for _, m := range state.Modules {
			if len(m.Resources) > 0 {
		...
		location := tpgresource.GetResourceNameFromSelfLink(rawState["location"])
		endpoint := tpgresource.GetResourceNameFromSelfLink(rawState["endpoint"])
		return acctest.BuildIAMImportId(fmt.Sprintf("projects/%s/locations/%s/endpoints/%s", project, location, endpoint), rawState["role"], "", rawState["condition.0.title"]), nil
	}
}

func generateVertexAIEndpointIAMMemberStateID(iamResourceAddr string) func(*terraform.State) (string, error) {
	return func(state *terraform.State) (string, error) {
		var rawState map[string]string
		for _, m := range state.Modules {
			if len(m.Resources) > 0 {
		...
		location := tpgresource.GetResourceNameFromSelfLink(rawState["location"])
		endpoint := tpgresource.GetResourceNameFromSelfLink(rawState["endpoint"])
		return acctest.BuildIAMImportId(fmt.Sprintf("projects/%s/locations/%s/endpoints/%s", project, location, endpoint), rawState["role"], rawState["member"], rawState["condition.0.title"]), nil
	}
}
