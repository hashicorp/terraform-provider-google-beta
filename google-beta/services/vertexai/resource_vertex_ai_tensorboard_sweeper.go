...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAITensorboard,
		DeleteFunction: testSweepVertexAITensorboard,
	}
...
func testSweepVertexAITensorboard(_ string) error {
	return listAndActionVertexAITensorboard(deleteResourceVertexAITensorboard)
}

func listAndActionVertexAITensorboard(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAITensorboard"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-aiplatform.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/tensorboards", "?")[0]
		...
		resourceList, ok := res["tensorboards"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'tensorboards'")
		} else {
...
func deleteResourceVertexAITensorboard(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-aiplatform.googleapis.com/v1beta1/{{name}}"
