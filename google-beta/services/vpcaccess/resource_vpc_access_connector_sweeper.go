...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVPCAccessConnector,
		DeleteFunction: testSweepVPCAccessConnector,
	}
...
func testSweepVPCAccessConnector(_ string) error {
	return listAndActionVPCAccessConnector(deleteResourceVPCAccessConnector)
}

func listAndActionVPCAccessConnector(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VPCAccessConnector"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["connectors"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'connectors'")
		} else {
...
func deleteResourceVPCAccessConnector(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
