...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSourceRepoRepository,
		DeleteFunction: testSweepSourceRepoRepository,
	}
...
func testSweepSourceRepoRepository(_ string) error {
	return listAndActionSourceRepoRepository(deleteResourceSourceRepoRepository)
}

func listAndActionSourceRepoRepository(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SourceRepoRepository"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://sourcerepo.googleapis.com/v1/projects/{{project}}/repos", "?")[0]
		...
		resourceList, ok := res["repos"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'repos'")
		} else {
...
func deleteResourceSourceRepoRepository(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://sourcerepo.googleapis.com/v1/projects/{{project}}/repos/{{name}}"
