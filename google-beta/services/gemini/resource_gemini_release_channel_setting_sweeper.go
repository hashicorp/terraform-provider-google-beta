...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_gemini_release_channel_setting",
		ListAndAction:  listAndActionGeminiReleaseChannelSetting,
		DeleteFunction: testSweepGeminiReleaseChannelSetting,
	}
...
func testSweepGeminiReleaseChannelSetting(_ string) error {
	return listAndActionGeminiReleaseChannelSetting(deleteResourceGeminiReleaseChannelSetting)
}

func listAndActionGeminiReleaseChannelSetting(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GeminiReleaseChannelSetting"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://cloudaicompanion.googleapis.com/v1/projects/{{project}}/locations/{{location}}/releaseChannelSettings", "?")[0]
		...
		resourceList, ok := res["releaseChannelSettings"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'releaseChannelSettings'")
		} else {
...
func deleteResourceGeminiReleaseChannelSetting(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://cloudaicompanion.googleapis.com/v1/projects/{{project}}/locations/{{location}}/releaseChannelSettings/{{release_channel_setting_id}}"
