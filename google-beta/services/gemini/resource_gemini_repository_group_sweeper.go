...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionGeminiRepositoryGroup,
		DeleteFunction: testSweepGeminiRepositoryGroup,
	}
	// Add parent relationship
	s.Parents = []string{"google_gemini_code_repository_index"}

	// Register the sweeper
	sweeper.AddTestSweepers(s)
}

func testSweepGeminiRepositoryGroup(_ string) error {
	return listAndActionGeminiRepositoryGroup(deleteResourceGeminiRepositoryGroup)
}

func listAndActionGeminiRepositoryGroup(action sweeper.ResourceAction) error {
	var lastError error
	...
	parentType := "google_gemini_code_repository_index"
	...
	if err := parentSweeper.ListAndAction(collectParentConfigGeminiRepositoryGroup(&configs)); err != nil {
	...
	for _, mockConfig := range configs {
		...
		parentValue := ""
		if v, ok := mockConfig.FieldsInSchema["code_repository_index"]; ok {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'repositoryGroups'")
		} else {
...
func deleteResourceGeminiRepositoryGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
...
// collectParentConfigGeminiRepositoryGroup returns a function that collects parent configurations
func collectParentConfigGeminiRepositoryGroup(configs *[]*tpgresource.ResourceDataMock) sweeper.ResourceAction {
	return func(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, parentObj map[string]interface{}) error {
		...
		if parentObj["name"] == nil {
			log.Printf("[INFO][SWEEPER_LOG] Parent google_gemini_code_repository_index field name was nil, skipping")
			return nil
		}
		...
		childConfig.FieldsInSchema["code_repository_index"] = parentValue
