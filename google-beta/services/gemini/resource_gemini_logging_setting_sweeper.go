...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_gemini_logging_setting",
		ListAndAction:  listAndActionGeminiLoggingSetting,
		DeleteFunction: testSweepGeminiLoggingSetting,
	}
...
func testSweepGeminiLoggingSetting(_ string) error {
	return listAndActionGeminiLoggingSetting(deleteResourceGeminiLoggingSetting)
}

func listAndActionGeminiLoggingSetting(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://cloudaicompanion.googleapis.com/v1/projects/{{project}}/locations/{{location}}/loggingSettings", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'loggingSettings'")
		} else {
...
func deleteResourceGeminiLoggingSetting(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://cloudaicompanion.googleapis.com/v1/projects/{{project}}/locations/{{location}}/loggingSettings/{{logging_setting_id}}"
