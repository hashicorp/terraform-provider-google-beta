...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_gemini_code_tools_setting",
		ListAndAction:  listAndActionGeminiCodeToolsSetting,
		DeleteFunction: testSweepGeminiCodeToolsSetting,
	}
...
func testSweepGeminiCodeToolsSetting(_ string) error {
	return listAndActionGeminiCodeToolsSetting(deleteResourceGeminiCodeToolsSetting)
}

func listAndActionGeminiCodeToolsSetting(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GeminiCodeToolsSetting"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://cloudaicompanion.googleapis.com/v1/projects/{{project}}/locations/{{location}}/codeToolsSettings", "?")[0]
		...
		resourceList, ok := res["codeToolsSettings"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'codeToolsSettings'")
		} else {
...
func deleteResourceGeminiCodeToolsSetting(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://cloudaicompanion.googleapis.com/v1/projects/{{project}}/locations/{{location}}/codeToolsSettings/{{code_tools_setting_id}}"
