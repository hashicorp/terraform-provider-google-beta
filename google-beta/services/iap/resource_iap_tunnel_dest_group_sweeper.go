...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionIapTunnelDestGroup,
		DeleteFunction: testSweepIapTunnelDestGroup,
	}
...
func testSweepIapTunnelDestGroup(_ string) error {
	return listAndActionIapTunnelDestGroup(deleteResourceIapTunnelDestGroup)
}

func listAndActionIapTunnelDestGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "IapTunnelDestGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://iap.googleapis.com/v1/projects/{{project}}/iap_tunnel/locations/{{region}}/destGroups", "?")[0]
		...
		resourceList, ok := res["tunnelDestGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'tunnelDestGroups'")
		} else {
...
func deleteResourceIapTunnelDestGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://iap.googleapis.com/v1/projects/{{project}}/iap_tunnel/locations/{{region}}/destGroups/{{group_name}}"
