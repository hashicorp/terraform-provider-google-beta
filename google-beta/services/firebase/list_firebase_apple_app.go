...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewFirebaseAppleAppListResource,
	}.Register()
}

var _ list.ListResource = &FirebaseAppleAppListResource{}

type FirebaseAppleAppListResource struct {
	tpgresource.ListResourceMetadata
}

func NewFirebaseAppleAppListResource() list.ListResource {
	listR := &FirebaseAppleAppListResource{}
	listR.TypeName = "google_firebase_apple_app"
	listR.SDKv2Resource = ResourceFirebaseAppleApp()
...
// FirebaseAppleAppListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type FirebaseAppleAppListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *FirebaseAppleAppListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data FirebaseAppleAppListModel
...
func ListFirebaseAppleApps(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceFirebaseAppleApp().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{FirebaseBasePath}}projects/{{project}}/iosApps")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceFirebaseAppleApp(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceFirebaseAppleAppFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
