...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewFirebaseAndroidAppListResource,
	}.Register()
}

var _ list.ListResource = &FirebaseAndroidAppListResource{}

type FirebaseAndroidAppListResource struct {
	tpgresource.ListResourceMetadata
}

func NewFirebaseAndroidAppListResource() list.ListResource {
	listR := &FirebaseAndroidAppListResource{}
	listR.TypeName = "google_firebase_android_app"
	listR.SDKv2Resource = ResourceFirebaseAndroidApp()
...
// FirebaseAndroidAppListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type FirebaseAndroidAppListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *FirebaseAndroidAppListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data FirebaseAndroidAppListModel
...
func ListFirebaseAndroidApps(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceFirebaseAndroidApp().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{FirebaseBasePath}}projects/{{project}}/androidApps")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceFirebaseAndroidApp(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceFirebaseAndroidAppFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
