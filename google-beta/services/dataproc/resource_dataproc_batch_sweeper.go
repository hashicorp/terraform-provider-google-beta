...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataprocBatch,
		DeleteFunction: testSweepDataprocBatch,
	}
...
func testSweepDataprocBatch(_ string) error {
	return listAndActionDataprocBatch(deleteResourceDataprocBatch)
}

func listAndActionDataprocBatch(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'batches'")
		} else {
...
func deleteResourceDataprocBatch(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
