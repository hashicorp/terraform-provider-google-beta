...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionApphubApplication,
		DeleteFunction: testSweepApphubApplication,
	}
...
func testSweepApphubApplication(_ string) error {
	return listAndActionApphubApplication(deleteResourceApphubApplication)
}

func listAndActionApphubApplication(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://apphub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/applications", "?")[0]
...
func deleteResourceApphubApplication(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
