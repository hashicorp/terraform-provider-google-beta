...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataplexMetadataFeed,
		DeleteFunction: testSweepDataplexMetadataFeed,
	}
...
func testSweepDataplexMetadataFeed(_ string) error {
	return listAndActionDataplexMetadataFeed(deleteResourceDataplexMetadataFeed)
}

func listAndActionDataplexMetadataFeed(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DataplexMetadataFeed"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://dataplex.googleapis.com/v1/projects/{{project}}/locations/{{location}}/metadataFeeds", "?")[0]
		...
		resourceList, ok := res["metadataFeeds"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'metadataFeeds'")
		} else {
...
func deleteResourceDataplexMetadataFeed(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://dataplex.googleapis.com/v1/projects/{{project}}/locations/{{location}}/metadataFeeds/{{metadata_feed_id}}"
