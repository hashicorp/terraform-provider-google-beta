...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataplexDataProduct,
		DeleteFunction: testSweepDataplexDataProduct,
	}
...
func testSweepDataplexDataProduct(_ string) error {
	return listAndActionDataplexDataProduct(deleteResourceDataplexDataProduct)
}

func listAndActionDataplexDataProduct(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DataplexDataProduct"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://dataplex.googleapis.com/v1/projects/{{project}}/locations/{{location}}/dataProducts", "?")[0]
		...
		resourceList, ok := res["dataProducts"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'dataProducts'")
		} else {
...
func deleteResourceDataplexDataProduct(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://dataplex.googleapis.com/v1/projects/{{project}}/locations/{{location}}/dataProducts/{{data_product_id}}"
