...
func (w *DataplexOperationWaiter) QueryOp() (interface{}, error) {
	...
	url := fmt.Sprintf("%s%s", w.Config.DataplexBasePath, w.CommonOperationWaiter.Op.Name)
...
func createDataplexWaiter(config *transport_tpg.Config, op map[string]interface{}, project, activity, userAgent string) (*DataplexOperationWaiter, error) {
	w := &DataplexOperationWaiter{
		Config:    config,
		UserAgent: userAgent,
		Project:   project,
	}
...
func DataplexOperationWaitTimeWithResponse(config *transport_tpg.Config, op map[string]interface{}, response *map[string]interface{}, project, activity, userAgent string, timeout time.Duration) error {
	w, err := createDataplexWaiter(config, op, project, activity, userAgent)
	if err != nil {
		return err
	}
