...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataplexDatascan,
		DeleteFunction: testSweepDataplexDatascan,
	}
...
func testSweepDataplexDatascan(_ string) error {
	return listAndActionDataplexDatascan(deleteResourceDataplexDatascan)
}

func listAndActionDataplexDatascan(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DataplexDatascan"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://dataplex.googleapis.com/v1/projects/{{project}}/locations/{{location}}/dataScans", "?")[0]
		...
		resourceList, ok := res["dataScans"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'dataScans'")
		} else {
...
func deleteResourceDataplexDatascan(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://dataplex.googleapis.com/v1/projects/{{project}}/locations/{{location}}/dataScans/{{data_scan_id}}"
