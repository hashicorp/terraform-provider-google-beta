...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_apihub_curation",
		ListAndAction:  listAndActionApihubCuration,
		DeleteFunction: testSweepApihubCuration,
	}
...
func testSweepApihubCuration(_ string) error {
	return listAndActionApihubCuration(deleteResourceApihubCuration)
}

func listAndActionApihubCuration(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ApihubCuration"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/curations", "?")[0]
		...
		resourceList, ok := res["curations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'curations'")
		} else {
...
func deleteResourceApihubCuration(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://apihub.googleapis.com/v1/projects/{{project}}/locations/{{location}}/curations/{{curation_id}}"
