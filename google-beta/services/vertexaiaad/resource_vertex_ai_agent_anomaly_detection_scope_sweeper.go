...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVertexAiAadAgentAnomalyDetectionScope,
		DeleteFunction: testSweepVertexAiAadAgentAnomalyDetectionScope,
	}
...
func testSweepVertexAiAadAgentAnomalyDetectionScope(_ string) error {
	return listAndActionVertexAiAadAgentAnomalyDetectionScope(deleteResourceVertexAiAadAgentAnomalyDetectionScope)
}

func listAndActionVertexAiAadAgentAnomalyDetectionScope(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VertexAiAadAgentAnomalyDetectionScope"
	...
	intermediateValues[0]["region"] = "us"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://aiplatform.{{region}}.rep.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/agentAnomalyDetectionScopes", "?")[0]
		...
		resourceList, ok := res["agentAnomalyDetectionScopes"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'agentAnomalyDetectionScopes'")
		} else {
...
func deleteResourceVertexAiAadAgentAnomalyDetectionScope(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://aiplatform.{{region}}.rep.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/agentAnomalyDetectionScopes/{{agent_anomaly_detection_scope_id}}"
