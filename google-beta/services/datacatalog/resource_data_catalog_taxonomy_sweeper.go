...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataCatalogTaxonomy,
		DeleteFunction: testSweepDataCatalogTaxonomy,
	}
...
func testSweepDataCatalogTaxonomy(_ string) error {
	return listAndActionDataCatalogTaxonomy(deleteResourceDataCatalogTaxonomy)
}

func listAndActionDataCatalogTaxonomy(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'taxonomies'")
		} else {
...
func deleteResourceDataCatalogTaxonomy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
