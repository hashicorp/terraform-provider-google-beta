...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionWorkbenchInstance,
		DeleteFunction: testSweepWorkbenchInstance,
	}
...
func testSweepWorkbenchInstance(_ string) error {
	return listAndActionWorkbenchInstance(deleteResourceWorkbenchInstance)
}

func listAndActionWorkbenchInstance(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "WorkbenchInstance"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://notebooks.googleapis.com/v2/projects/{{project}}/locations/{{location}}/instances", "?")[0]
...
func deleteResourceWorkbenchInstance(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://notebooks.googleapis.com/v2/projects/{{project}}/locations/{{location}}/instances/{{name}}"
