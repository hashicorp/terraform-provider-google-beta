...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionCloudIdentityPolicy,
		DeleteFunction: testSweepCloudIdentityPolicy,
	}
...
func testSweepCloudIdentityPolicy(_ string) error {
	return listAndActionCloudIdentityPolicy(deleteResourceCloudIdentityPolicy)
}

func listAndActionCloudIdentityPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "CloudIdentityPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://cloudidentity.googleapis.com/v1beta1/policies", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'policies'")
		} else {
...
func deleteResourceCloudIdentityPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
