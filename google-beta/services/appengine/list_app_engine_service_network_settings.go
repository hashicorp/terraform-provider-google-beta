...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewAppEngineServiceNetworkSettingsListResource,
	}.Register()
}

var _ list.ListResource = &AppEngineServiceNetworkSettingsListResource{}

type AppEngineServiceNetworkSettingsListResource struct {
	tpgresource.ListResourceMetadata
}

func NewAppEngineServiceNetworkSettingsListResource() list.ListResource {
	listR := &AppEngineServiceNetworkSettingsListResource{}
	listR.TypeName = "google_app_engine_service_network_settings"
	listR.SDKv2Resource = ResourceAppEngineServiceNetworkSettings()
...
// AppEngineServiceNetworkSettingsListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type AppEngineServiceNetworkSettingsListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *AppEngineServiceNetworkSettingsListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data AppEngineServiceNetworkSettingsListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListAppEngineServiceNetworkSettingss(
			listR.Client,
			project,
...
func ListAppEngineServiceNetworkSettingss(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceAppEngineServiceNetworkSettings().Data(&terraform.InstanceState{})
	if project != "" {
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceAppEngineServiceNetworkSettings(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceAppEngineServiceNetworkSettingsFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
