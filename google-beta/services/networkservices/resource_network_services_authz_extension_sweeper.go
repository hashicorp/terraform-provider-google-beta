...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesAuthzExtension,
		DeleteFunction: testSweepNetworkServicesAuthzExtension,
	}
...
func testSweepNetworkServicesAuthzExtension(_ string) error {
	return listAndActionNetworkServicesAuthzExtension(deleteResourceNetworkServicesAuthzExtension)
}

func listAndActionNetworkServicesAuthzExtension(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesAuthzExtension"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/authzExtensions", "?")[0]
		...
		resourceList, ok := res["authzExtensions"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'authzExtensions'")
		} else {
...
func deleteResourceNetworkServicesAuthzExtension(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/authzExtensions/{{name}}"
