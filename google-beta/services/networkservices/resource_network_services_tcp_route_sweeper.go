...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_services_tcp_route",
		ListAndAction:  listAndActionNetworkServicesTcpRoute,
		DeleteFunction: testSweepNetworkServicesTcpRoute,
	}
...
func testSweepNetworkServicesTcpRoute(_ string) error {
	return listAndActionNetworkServicesTcpRoute(deleteResourceNetworkServicesTcpRoute)
}

func listAndActionNetworkServicesTcpRoute(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesTcpRoute"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/global/tcpRoutes", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'tcpRoutes'")
		} else {
...
func deleteResourceNetworkServicesTcpRoute(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/global/tcpRoutes/{{name}}"
