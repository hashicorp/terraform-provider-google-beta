...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewNetworkServicesAuthzExtensionListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesAuthzExtensionListResource{}

type NetworkServicesAuthzExtensionListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesAuthzExtensionListResource() list.ListResource {
	listR := &NetworkServicesAuthzExtensionListResource{}
	listR.TypeName = "google_network_services_authz_extension"
	listR.SDKv2Resource = ResourceNetworkServicesAuthzExtension()
...
// NetworkServicesAuthzExtensionListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesAuthzExtensionListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesAuthzExtensionListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesAuthzExtensionListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesAuthzExtensions(
			listR.Client,
			location,
			project,
...
func ListNetworkServicesAuthzExtensions(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesAuthzExtension().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/authzExtensions")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesAuthzExtension(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "authzExtensions",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceNetworkServicesAuthzExtensionFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
