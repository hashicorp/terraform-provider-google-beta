...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastGroupConsumerActivation,
...
func testSweepNetworkServicesMulticastGroupConsumerActivation(_ string) error {
	return listAndActionNetworkServicesMulticastGroupConsumerActivation(deleteResourceNetworkServicesMulticastGroupConsumerActivation)
}

func listAndActionNetworkServicesMulticastGroupConsumerActivation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastGroupConsumerActivation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupConsumerActivations", "?")[0]
		...
		resourceList, ok := res["multicastGroupConsumerActivations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastGroupConsumerActivations'")
		} else {
...
func deleteResourceNetworkServicesMulticastGroupConsumerActivation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupConsumerActivations/{{multicast_group_consumer_activation_id}}"
