...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesWasmPlugin,
		DeleteFunction: testSweepNetworkServicesWasmPlugin,
	}
...
func testSweepNetworkServicesWasmPlugin(_ string) error {
	return listAndActionNetworkServicesWasmPlugin(deleteResourceNetworkServicesWasmPlugin)
}

func listAndActionNetworkServicesWasmPlugin(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesWasmPlugin"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/wasmPlugins", "?")[0]
		...
		resourceList, ok := res["wasmPlugins"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'wasmPlugins'")
		} else {
...
func deleteResourceNetworkServicesWasmPlugin(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/wasmPlugins/{{name}}"
