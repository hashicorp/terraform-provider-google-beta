...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesEdgeCacheOrigin,
		DeleteFunction: testSweepNetworkServicesEdgeCacheOrigin,
	}
...
func testSweepNetworkServicesEdgeCacheOrigin(_ string) error {
	return listAndActionNetworkServicesEdgeCacheOrigin(deleteResourceNetworkServicesEdgeCacheOrigin)
}

func listAndActionNetworkServicesEdgeCacheOrigin(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesEdgeCacheOrigin"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/global/edgeCacheOrigins", "?")[0]
		...
		resourceList, ok := res["edgeCacheOrigins"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'edgeCacheOrigins'")
		} else {
...
func deleteResourceNetworkServicesEdgeCacheOrigin(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/global/edgeCacheOrigins/{{name}}"
