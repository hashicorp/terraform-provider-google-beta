...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastConsumerAssociation,
		DeleteFunction: testSweepNetworkServicesMulticastConsumerAssociation,
	}
...
func testSweepNetworkServicesMulticastConsumerAssociation(_ string) error {
	return listAndActionNetworkServicesMulticastConsumerAssociation(deleteResourceNetworkServicesMulticastConsumerAssociation)
}

func listAndActionNetworkServicesMulticastConsumerAssociation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastConsumerAssociation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastConsumerAssociations", "?")[0]
		...
		resourceList, ok := res["multicastConsumerAssociations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastConsumerAssociations'")
		} else {
...
func deleteResourceNetworkServicesMulticastConsumerAssociation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastConsumerAssociations/{{multicast_consumer_association_id}}"
