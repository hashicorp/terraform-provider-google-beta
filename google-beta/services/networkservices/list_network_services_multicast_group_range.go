...
func init() {
	registry.FrameworkListResource{
		Name:        "google_network_services_multicast_group_range",
		ProductName: "NetworkServices",
		Func:        NewNetworkServicesMulticastGroupRangeListResource,
	}.Register()
}

var _ list.ListResource = &NetworkServicesMulticastGroupRangeListResource{}

type NetworkServicesMulticastGroupRangeListResource struct {
	tpgresource.ListResourceMetadata
}

func NewNetworkServicesMulticastGroupRangeListResource() list.ListResource {
	listR := &NetworkServicesMulticastGroupRangeListResource{}
	listR.TypeName = "google_network_services_multicast_group_range"
	listR.SDKv2Resource = ResourceNetworkServicesMulticastGroupRange()
...
// NetworkServicesMulticastGroupRangeListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type NetworkServicesMulticastGroupRangeListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *NetworkServicesMulticastGroupRangeListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkServicesMulticastGroupRangeListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListNetworkServicesMulticastGroupRanges(
			listR.Client,
			location,
			project,
			func(rd *schema.ResourceData) error {
				...
				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "multicast_group_range_id"); err != nil {
					return err
				}
...
func ListNetworkServicesMulticastGroupRanges(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceNetworkServicesMulticastGroupRange().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{NetworkServicesBasePath}}projects/{{project}}/locations/{{location}}/multicastGroupRanges")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceNetworkServicesMulticastGroupRange(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "multicastGroupRanges",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if v, ok := res["multicastGroupRangeId"]; ok && v != nil {
				if err := d.Set("multicast_group_range_id", v); err != nil {
					return fmt.Errorf("error setting multicast_group_range_id: %w", err)
				}
			}
			if err = ResourceNetworkServicesMulticastGroupRangeFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
