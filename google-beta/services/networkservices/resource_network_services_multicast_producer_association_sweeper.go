...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastProducerAssociation,
		DeleteFunction: testSweepNetworkServicesMulticastProducerAssociation,
	}
...
func testSweepNetworkServicesMulticastProducerAssociation(_ string) error {
	return listAndActionNetworkServicesMulticastProducerAssociation(deleteResourceNetworkServicesMulticastProducerAssociation)
}

func listAndActionNetworkServicesMulticastProducerAssociation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastProducerAssociation"
	...
	intermediateValues[0]["location"] = "us-central1-b"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastProducerAssociations", "?")[0]
		...
		resourceList, ok := res["multicastProducerAssociations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastProducerAssociations'")
		} else {
...
func deleteResourceNetworkServicesMulticastProducerAssociation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastProducerAssociations/{{multicast_producer_association_id}}"
