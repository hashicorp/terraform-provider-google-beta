...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesEdgeCacheKeyset,
		DeleteFunction: testSweepNetworkServicesEdgeCacheKeyset,
	}
...
func testSweepNetworkServicesEdgeCacheKeyset(_ string) error {
	return listAndActionNetworkServicesEdgeCacheKeyset(deleteResourceNetworkServicesEdgeCacheKeyset)
}

func listAndActionNetworkServicesEdgeCacheKeyset(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'edgeCacheKeysets'")
		} else {
...
func deleteResourceNetworkServicesEdgeCacheKeyset(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
