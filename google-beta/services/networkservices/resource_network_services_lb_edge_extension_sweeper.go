...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_services_lb_edge_extension",
		ListAndAction:  listAndActionNetworkServicesLbEdgeExtension,
		DeleteFunction: testSweepNetworkServicesLbEdgeExtension,
	}
...
func testSweepNetworkServicesLbEdgeExtension(_ string) error {
	return listAndActionNetworkServicesLbEdgeExtension(deleteResourceNetworkServicesLbEdgeExtension)
}

func listAndActionNetworkServicesLbEdgeExtension(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesLbEdgeExtension"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/lbEdgeExtensions", "?")[0]
		...
		resourceList, ok := res["lbEdgeExtensions"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'lbEdgeExtensions'")
		} else {
...
func deleteResourceNetworkServicesLbEdgeExtension(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/lbEdgeExtensions/{{name}}"
