...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_services_grpc_route",
		ListAndAction:  listAndActionNetworkServicesGrpcRoute,
		DeleteFunction: testSweepNetworkServicesGrpcRoute,
	}
...
func testSweepNetworkServicesGrpcRoute(_ string) error {
	return listAndActionNetworkServicesGrpcRoute(deleteResourceNetworkServicesGrpcRoute)
}

func listAndActionNetworkServicesGrpcRoute(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesGrpcRoute"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'grpcRoutes'")
		} else {
...
func deleteResourceNetworkServicesGrpcRoute(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
