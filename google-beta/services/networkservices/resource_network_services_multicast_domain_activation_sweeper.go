...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastDomainActivation,
		DeleteFunction: testSweepNetworkServicesMulticastDomainActivation,
	}
...
func testSweepNetworkServicesMulticastDomainActivation(_ string) error {
	return listAndActionNetworkServicesMulticastDomainActivation(deleteResourceNetworkServicesMulticastDomainActivation)
}

func listAndActionNetworkServicesMulticastDomainActivation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastDomainActivation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastDomainActivations", "?")[0]
		...
		resourceList, ok := res["multicastDomainActivations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastDomainActivations'")
		} else {
...
func deleteResourceNetworkServicesMulticastDomainActivation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastDomainActivations/{{multicast_domain_activation_id}}"
