...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastGroupProducerActivation,
...
func testSweepNetworkServicesMulticastGroupProducerActivation(_ string) error {
	return listAndActionNetworkServicesMulticastGroupProducerActivation(deleteResourceNetworkServicesMulticastGroupProducerActivation)
}

func listAndActionNetworkServicesMulticastGroupProducerActivation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastGroupProducerActivation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupProducerActivations", "?")[0]
		...
		resourceList, ok := res["multicastGroupProducerActivations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastGroupProducerActivations'")
		} else {
...
func deleteResourceNetworkServicesMulticastGroupProducerActivation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupProducerActivations/{{multicast_group_producer_activation_id}}"
