...
func TestAccNetworkServicesMulticastGroupRangeListQuery_generated(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_network_services_multicast_group_range.mgr_test",
						[]string{
							"name",
					...
					listScope.Capture(map[string]string{
						"location": "google_network_services_multicast_group_range.mgr_test",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccNetworkServicesMulticastGroupRange_networkServicesMulticastGroupRangeBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_network_services_multicast_group_range.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_network_services_multicast_group_range.list_query",
...
func testAccNetworkServicesMulticastGroupRange_networkServicesMulticastGroupRangeBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_network_services_multicast_group_range" "list_query" {
    provider = google
	limit = 10000
    config {
