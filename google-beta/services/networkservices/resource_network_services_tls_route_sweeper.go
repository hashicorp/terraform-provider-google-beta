...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_services_tls_route",
		ListAndAction:  listAndActionNetworkServicesTlsRoute,
		DeleteFunction: testSweepNetworkServicesTlsRoute,
	}
...
func testSweepNetworkServicesTlsRoute(_ string) error {
	return listAndActionNetworkServicesTlsRoute(deleteResourceNetworkServicesTlsRoute)
}

func listAndActionNetworkServicesTlsRoute(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesTlsRoute"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["tlsRoutes"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'tlsRoutes'")
		} else {
...
func deleteResourceNetworkServicesTlsRoute(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
