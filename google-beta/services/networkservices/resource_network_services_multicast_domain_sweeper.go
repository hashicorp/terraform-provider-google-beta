...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastDomain,
		DeleteFunction: testSweepNetworkServicesMulticastDomain,
	}
...
func testSweepNetworkServicesMulticastDomain(_ string) error {
	return listAndActionNetworkServicesMulticastDomain(deleteResourceNetworkServicesMulticastDomain)
}

func listAndActionNetworkServicesMulticastDomain(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastDomain"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastDomains", "?")[0]
		...
		resourceList, ok := res["multicastDomains"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastDomains'")
		} else {
...
func deleteResourceNetworkServicesMulticastDomain(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastDomains/{{multicast_domain_id}}"
