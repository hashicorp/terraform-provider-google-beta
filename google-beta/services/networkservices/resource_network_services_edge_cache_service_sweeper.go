...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesEdgeCacheService,
		DeleteFunction: testSweepNetworkServicesEdgeCacheService,
	}
...
func testSweepNetworkServicesEdgeCacheService(_ string) error {
	return listAndActionNetworkServicesEdgeCacheService(deleteResourceNetworkServicesEdgeCacheService)
}

func listAndActionNetworkServicesEdgeCacheService(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesEdgeCacheService"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/global/edgeCacheServices", "?")[0]
		...
		resourceList, ok := res["edgeCacheServices"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'edgeCacheServices'")
		} else {
...
func deleteResourceNetworkServicesEdgeCacheService(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/global/edgeCacheServices/{{name}}"
