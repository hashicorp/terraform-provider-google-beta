...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesAgentGateway,
		DeleteFunction: testSweepNetworkServicesAgentGateway,
	}
...
func testSweepNetworkServicesAgentGateway(_ string) error {
	return listAndActionNetworkServicesAgentGateway(deleteResourceNetworkServicesAgentGateway)
}

func listAndActionNetworkServicesAgentGateway(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesAgentGateway"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/agentGateways", "?")[0]
		...
		resourceList, ok := res["agentGateways"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'agentGateways'")
		} else {
...
func deleteResourceNetworkServicesAgentGateway(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/agentGateways/{{name}}"
