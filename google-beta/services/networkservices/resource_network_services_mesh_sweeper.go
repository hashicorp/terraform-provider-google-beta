...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_network_services_mesh",
		ListAndAction:  listAndActionNetworkServicesMesh,
		DeleteFunction: testSweepNetworkServicesMesh,
	}
...
func testSweepNetworkServicesMesh(_ string) error {
	return listAndActionNetworkServicesMesh(deleteResourceNetworkServicesMesh)
}

func listAndActionNetworkServicesMesh(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMesh"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["meshes"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'meshes'")
		} else {
...
func deleteResourceNetworkServicesMesh(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
