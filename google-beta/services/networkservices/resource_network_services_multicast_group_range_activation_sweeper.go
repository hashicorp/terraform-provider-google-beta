...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkServicesMulticastGroupRangeActivation,
		DeleteFunction: testSweepNetworkServicesMulticastGroupRangeActivation,
	}
...
func testSweepNetworkServicesMulticastGroupRangeActivation(_ string) error {
	return listAndActionNetworkServicesMulticastGroupRangeActivation(deleteResourceNetworkServicesMulticastGroupRangeActivation)
}

func listAndActionNetworkServicesMulticastGroupRangeActivation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkServicesMulticastGroupRangeActivation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupRangeActivations", "?")[0]
		...
		resourceList, ok := res["multicastGroupRangeActivations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'multicastGroupRangeActivations'")
		} else {
...
func deleteResourceNetworkServicesMulticastGroupRangeActivation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkservices.googleapis.com/v1/projects/{{project}}/locations/{{location}}/multicastGroupRangeActivations/{{multicast_group_range_activation_id}}"
