...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMemorystoreAclPolicy,
		DeleteFunction: testSweepMemorystoreAclPolicy,
	}
...
func testSweepMemorystoreAclPolicy(_ string) error {
	return listAndActionMemorystoreAclPolicy(deleteResourceMemorystoreAclPolicy)
}

func listAndActionMemorystoreAclPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MemorystoreAclPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://memorystore.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/aclPolicies", "?")[0]
		...
		resourceList, ok := res["aclPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'aclPolicies'")
		} else {
...
func deleteResourceMemorystoreAclPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://memorystore.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/aclPolicies/{{acl_policy_id}}"
