...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionGkeonpremBareMetalCluster,
		DeleteFunction: testSweepGkeonpremBareMetalCluster,
	}
...
func testSweepGkeonpremBareMetalCluster(_ string) error {
	return listAndActionGkeonpremBareMetalCluster(deleteResourceGkeonpremBareMetalCluster)
}

func listAndActionGkeonpremBareMetalCluster(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://gkeonprem.googleapis.com/v1/projects/{{project}}/locations/{{location}}/bareMetalClusters", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'bareMetalClusters'")
		} else {
...
func deleteResourceGkeonpremBareMetalCluster(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://gkeonprem.googleapis.com/v1/projects/{{project}}/locations/{{location}}/bareMetalClusters/{{name}}?force=true"
