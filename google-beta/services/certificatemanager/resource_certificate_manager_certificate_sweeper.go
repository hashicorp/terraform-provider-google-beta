...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionCertificateManagerCertificate,
		DeleteFunction: testSweepCertificateManagerCertificate,
	}
...
func testSweepCertificateManagerCertificate(_ string) error {
	return listAndActionCertificateManagerCertificate(deleteResourceCertificateManagerCertificate)
}

func listAndActionCertificateManagerCertificate(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[2]["region"] = "us-west2"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'certificates'")
		} else {
...
func deleteResourceCertificateManagerCertificate(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
