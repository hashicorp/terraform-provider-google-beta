...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVectorSearchCollection,
		DeleteFunction: testSweepVectorSearchCollection,
	}
...
func testSweepVectorSearchCollection(_ string) error {
	return listAndActionVectorSearchCollection(deleteResourceVectorSearchCollection)
}

func listAndActionVectorSearchCollection(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "VectorSearchCollection"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://vectorsearch.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/collections", "?")[0]
		...
		resourceList, ok := res["collections"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'collections'")
		} else {
...
func deleteResourceVectorSearchCollection(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://vectorsearch.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/collections/{{collection_id}}"
