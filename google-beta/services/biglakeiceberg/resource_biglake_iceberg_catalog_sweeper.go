...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBiglakeIcebergIcebergCatalog,
		DeleteFunction: testSweepBiglakeIcebergIcebergCatalog,
	}
...
func testSweepBiglakeIcebergIcebergCatalog(_ string) error {
	return listAndActionBiglakeIcebergIcebergCatalog(deleteResourceBiglakeIcebergIcebergCatalog)
}

func listAndActionBiglakeIcebergIcebergCatalog(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BiglakeIcebergIcebergCatalog"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://biglake.googleapis.com/iceberg/v1/restcatalog/extensions/projects/{{project}}/catalogs", "?")[0]
		...
		resourceList, ok := res["icebergCatalogs"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'icebergCatalogs'")
		} else {
...
func deleteResourceBiglakeIcebergIcebergCatalog(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://biglake.googleapis.com/iceberg/v1/restcatalog/extensions/projects/{{project}}/catalogs/{{name}}"
