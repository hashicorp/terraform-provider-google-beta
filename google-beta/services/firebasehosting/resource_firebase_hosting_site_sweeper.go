...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_firebase_hosting_site",
		ListAndAction:  listAndActionFirebaseHostingSite,
		DeleteFunction: testSweepFirebaseHostingSite,
	}
...
func testSweepFirebaseHostingSite(_ string) error {
	return listAndActionFirebaseHostingSite(deleteResourceFirebaseHostingSite)
}

func listAndActionFirebaseHostingSite(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "FirebaseHostingSite"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://firebasehosting.googleapis.com/v1beta1/projects/{{project}}/sites", "?")[0]
		...
		resourceList, ok := res["sites"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'sites'")
		} else {
...
func deleteResourceFirebaseHostingSite(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://firebasehosting.googleapis.com/v1beta1/projects/{{project}}/sites/{{site_id}}"
