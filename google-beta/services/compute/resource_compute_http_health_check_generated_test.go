...
func TestAccComputeHttpHealthCheck_httpHealthCheckBasicExample(t *testing.T) {
	...
	context := map[string]interface{}{
		"http_health_check_name": "tf-test-authentication-health-check" + randomSuffix,
...
func testAccCheckComputeHttpHealthCheckDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(compute.Product, config), "projects/{{project}}/global/httpHealthChecks/{{name}}"))
			if err != nil {
				return err
			}
