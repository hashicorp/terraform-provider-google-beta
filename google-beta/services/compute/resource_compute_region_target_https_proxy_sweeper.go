...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeRegionTargetHttpsProxy,
		DeleteFunction: testSweepComputeRegionTargetHttpsProxy,
	}
...
func testSweepComputeRegionTargetHttpsProxy(_ string) error {
	return listAndActionComputeRegionTargetHttpsProxy(deleteResourceComputeRegionTargetHttpsProxy)
}

func listAndActionComputeRegionTargetHttpsProxy(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["targetHttpsProxies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'targetHttpsProxies'")
		} else {
...
func deleteResourceComputeRegionTargetHttpsProxy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
