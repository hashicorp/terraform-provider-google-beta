...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeInstantSnapshot,
		DeleteFunction: testSweepComputeInstantSnapshot,
	}
...
func testSweepComputeInstantSnapshot(_ string) error {
	return listAndActionComputeInstantSnapshot(deleteResourceComputeInstantSnapshot)
}

func listAndActionComputeInstantSnapshot(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeInstantSnapshot"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/aggregated/instantSnapshots", "?")[0]
...
func deleteResourceComputeInstantSnapshot(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/zones/{{zone}}/instantSnapshots/{{name}}"
	if obj["zone"] == nil {
