...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeRegionHealthCheck,
		DeleteFunction: testSweepComputeRegionHealthCheck,
	}
...
func testSweepComputeRegionHealthCheck(_ string) error {
	return listAndActionComputeRegionHealthCheck(deleteResourceComputeRegionHealthCheck)
}

func listAndActionComputeRegionHealthCheck(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/healthChecks", "?")[0]
...
func deleteResourceComputeRegionHealthCheck(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/healthChecks/{{name}}"
