...
func TestAccComputeSubnetwork_secondaryIpv6Ranges(t *testing.T) {
	t.Parallel()

	var subnetwork compute.Subnetwork
	cnName := fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))
	subnetworkName := fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))
	papName := fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))
	pdpName := fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))
	subPdpName := fmt.Sprintf("tf-test-%s", acctest.RandString(t, 10))
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Step 1: Create a normal custom subnet
				Config: testAccComputeSubnetwork_secondaryIpv6RangesBase(cnName, subnetworkName, papName, pdpName, subPdpName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeSubnetworkExists(t, "google_compute_subnetwork.ipv6-secondary-ranges", &subnetwork),
				),
			},
			{
				// Step 2: Add 3 secondary ranges to the subnet: 1 ULA, 2 BYOGUA (1 auto, 1 manual)
				Config: testAccComputeSubnetwork_secondaryIpv6RangesAdd(cnName, subnetworkName, papName, pdpName, subPdpName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeSubnetworkExists(t, "google_compute_subnetwork.ipv6-secondary-ranges", &subnetwork),
					testAccCheckComputeSubnetworkHasSecondaryIpv6Range(&subnetwork, "v6-ula", "IPV6", ""),
					testAccCheckComputeSubnetworkHasSecondaryIpv6Range(&subnetwork, "v6-byoip-auto", "IPV6", ""),
					testAccCheckComputeSubnetworkHasSecondaryIpv6Range(&subnetwork, "v6-byoip-manual", "IPV6", ""),
				),
			},
			{
				// Step 3: Change the order of the secondary ranges (Verifies no-op/no re-creation)
				Config: testAccComputeSubnetwork_secondaryIpv6RangesReorder(cnName, subnetworkName, papName, pdpName, subPdpName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeSubnetworkExists(t, "google_compute_subnetwork.ipv6-secondary-ranges", &subnetwork),
					testAccCheckComputeSubnetworkHasSecondaryIpv6Range(&subnetwork, "v6-ula", "IPV6", ""),
					testAccCheckComputeSubnetworkHasSecondaryIpv6Range(&subnetwork, "v6-byoip-auto", "IPV6", ""),
			...
			{
				// Step 4: Remove 2 ranges (ULA and Manual), leave 1 (Auto)
				Config: testAccComputeSubnetwork_secondaryIpv6RangesRemove(cnName, subnetworkName, papName, pdpName, subPdpName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeSubnetworkExists(t, "google_compute_subnetwork.ipv6-secondary-ranges", &subnetwork),
					testAccCheckComputeSubnetworkHasSecondaryIpv6Range(&subnetwork, "v6-byoip-auto", "IPV6", ""),
					testAccCheckComputeSubnetworkHasNotSecondaryIpRange(&subnetwork, "v6-ula", ""),
					testAccCheckComputeSubnetworkHasNotSecondaryIpRange(&subnetwork, "v6-byoip-manual", ""),
				),
			},
		},
	})
...
func testAccCheckComputeSubnetworkHasSecondaryIpv6Range(subnetwork *compute.Subnetwork, rangeName, ipVersion, ipCollection string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		for _, secondaryRange := range subnetwork.SecondaryIpRanges {
			if secondaryRange.RangeName == rangeName {
				if secondaryRange.IpVersion != ipVersion {
					return fmt.Errorf("Secondary range %s has wrong ipVersion. Expected %s, got %s", rangeName, ipVersion, secondaryRange.IpVersion)
				}
				if ipCollection != "" && secondaryRange.IpCollection != ipCollection {
					return fmt.Errorf("Secondary range %s has wrong ipCollection. Expected %s, got %s", rangeName, ipCollection, secondaryRange.IpCollection)
				}
				return nil
			}
		}
		return fmt.Errorf("Secondary IPv6 range %s not found", rangeName)
	}
}
...
func testAccComputeSubnetwork_secondaryIpv6RangesShared(cnName, papName, pdpName, subPdpName string) string {
	return fmt.Sprintf(`
resource "google_compute_network" "custom-test" {
...
resource "google_compute_public_delegated_prefix" "ipv6_pdp" {
  ...
  name          = "%s"
  region        = "us-central1"
  ip_cidr_range = "2001:db8:abcd::/48"
...
resource "google_compute_public_delegated_prefix" "ipv6_sub_pdp" {
  ...
  name          = "%s"
  region        = "us-central1"
  ip_cidr_range = "2001:db8:abcd::/56"
  parent_prefix = google_compute_public_delegated_prefix.ipv6_pdp.id
  mode          = "INTERNAL_IPV6_SUBNETWORK_CREATION"
}
`, cnName, papName, pdpName, subPdpName)
}

func testAccComputeSubnetwork_secondaryIpv6RangesBase(cnName, subnetworkName, papName, pdpName, subPdpName string) string {
	return fmt.Sprintf(`
%s
resource "google_compute_subnetwork" "ipv6-secondary-ranges" {
  provider         = google-beta
  name             = "%s"
  region           = "us-central1"
  network          = google_compute_network.custom-test.self_link
...
`, testAccComputeSubnetwork_secondaryIpv6RangesShared(cnName, papName, pdpName, subPdpName), subnetworkName)
}

func testAccComputeSubnetwork_secondaryIpv6RangesAdd(cnName, subnetworkName, papName, pdpName, subPdpName string) string {
	return fmt.Sprintf(`
%s
resource "google_compute_subnetwork" "ipv6-secondary-ranges" {
  provider         = google-beta
  ...
  secondary_ip_range { 
    range_name = "v6-byoip-auto"
    ip_version = "IPV6"
    ip_collection = google_compute_public_delegated_prefix.ipv6_sub_pdp.self_link
  }
  secondary_ip_range { 
    range_name = "v6-byoip-manual"
    ip_version = "IPV6"
    ip_collection = google_compute_public_delegated_prefix.ipv6_sub_pdp.self_link
    ip_cidr_range = "2001:db8:abcd:2::/64" 
  }
}
`, testAccComputeSubnetwork_secondaryIpv6RangesShared(cnName, papName, pdpName, subPdpName), subnetworkName)
}

func testAccComputeSubnetwork_secondaryIpv6RangesReorder(cnName, subnetworkName, papName, pdpName, subPdpName string) string {
	return fmt.Sprintf(`
%s
resource "google_compute_subnetwork" "ipv6-secondary-ranges" {
  provider         = google-beta
...
func testAccComputeSubnetwork_secondaryIpv6RangesRemove(cnName, subnetworkName, papName, pdpName, subPdpName string) string {
	return fmt.Sprintf(`
%s
resource "google_compute_subnetwork" "ipv6-secondary-ranges" {
  provider         = google-beta
