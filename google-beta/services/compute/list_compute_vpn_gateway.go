...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeVpnGatewayListResource,
	}.Register()
}

var _ list.ListResource = &ComputeVpnGatewayListResource{}

type ComputeVpnGatewayListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeVpnGatewayListResource() list.ListResource {
	listR := &ComputeVpnGatewayListResource{}
	listR.TypeName = "google_compute_vpn_gateway"
	listR.SDKv2Resource = ResourceComputeVpnGateway()
...
// ComputeVpnGatewayListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeVpnGatewayListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeVpnGatewayListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeVpnGatewayListModel
...
func ListComputeVpnGateways(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeVpnGateway().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/targetVpnGateways")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeVpnGateway(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeVpnGatewayFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
