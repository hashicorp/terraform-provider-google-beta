...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputePacketMirroring,
		DeleteFunction: testSweepComputePacketMirroring,
	}
...
func testSweepComputePacketMirroring(_ string) error {
	return listAndActionComputePacketMirroring(deleteResourceComputePacketMirroring)
}

func listAndActionComputePacketMirroring(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/packetMirrorings", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'packetMirrorings'")
		} else {
...
func deleteResourceComputePacketMirroring(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/packetMirrorings/{{name}}"
