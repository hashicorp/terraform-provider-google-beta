...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeHttpsHealthCheck,
		DeleteFunction: testSweepComputeHttpsHealthCheck,
	}
...
func testSweepComputeHttpsHealthCheck(_ string) error {
	return listAndActionComputeHttpsHealthCheck(deleteResourceComputeHttpsHealthCheck)
}

func listAndActionComputeHttpsHealthCheck(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeHttpsHealthCheck"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/httpsHealthChecks", "?")[0]
...
func deleteResourceComputeHttpsHealthCheck(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/httpsHealthChecks/{{name}}"
