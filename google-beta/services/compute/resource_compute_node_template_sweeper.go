...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeNodeTemplate,
		DeleteFunction: testSweepComputeNodeTemplate,
	}
...
func testSweepComputeNodeTemplate(_ string) error {
	return listAndActionComputeNodeTemplate(deleteResourceComputeNodeTemplate)
}

func listAndActionComputeNodeTemplate(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeNodeTemplate"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/nodeTemplates", "?")[0]
...
func deleteResourceComputeNodeTemplate(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/nodeTemplates/{{name}}"
