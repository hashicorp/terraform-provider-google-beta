...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeRegionHealthSource,
		DeleteFunction: testSweepComputeRegionHealthSource,
	}
...
func testSweepComputeRegionHealthSource(_ string) error {
	return listAndActionComputeRegionHealthSource(deleteResourceComputeRegionHealthSource)
}

func listAndActionComputeRegionHealthSource(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeRegionHealthSource"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/healthSources", "?")[0]
...
func deleteResourceComputeRegionHealthSource(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/healthSources/{{name}}"
