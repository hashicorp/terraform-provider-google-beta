...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeInstantSnapshotListResource,
	}.Register()
}

var _ list.ListResource = &ComputeInstantSnapshotListResource{}

type ComputeInstantSnapshotListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeInstantSnapshotListResource() list.ListResource {
	listR := &ComputeInstantSnapshotListResource{}
	listR.TypeName = "google_compute_instant_snapshot"
	listR.SDKv2Resource = ResourceComputeInstantSnapshot()
...
// ComputeInstantSnapshotListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeInstantSnapshotListModel struct {
	Zone    types.String `tfsdk:"zone"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeInstantSnapshotListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeInstantSnapshotListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeInstantSnapshots(
			listR.Client,
			zone,
			project,
...
func ListComputeInstantSnapshots(config *transport_tpg.Config,
	zone string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeInstantSnapshot().Data(&terraform.InstanceState{})
	if zone != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/zones/{{zone}}/instantSnapshots")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeInstantSnapshot(),
		ListURL:        url,
