...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeReservation,
		DeleteFunction: testSweepComputeReservation,
	}
...
func testSweepComputeReservation(_ string) error {
	return listAndActionComputeReservation(deleteResourceComputeReservation)
}

func listAndActionComputeReservation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeReservation"
	...
	intermediateValues[1]["zone"] = "us-west1-a"
	intermediateValues[2] = map[string]string{}
	intermediateValues[2]["zone"] = "us-central1-f"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/aggregated/reservations", "?")[0]
...
func deleteResourceComputeReservation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/zones/{{zone}}/reservations/{{name}}"
	if obj["zone"] == nil {
