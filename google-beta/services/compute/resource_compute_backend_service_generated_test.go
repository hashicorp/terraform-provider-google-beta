...
func TestAccComputeBackendService_backendServiceInFlightExample(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeBackendService_backendServiceInFlightExample(context),
			},
			{
				ResourceName:            "google_compute_backend_service.default",
				ImportState:             true,
...
func testAccComputeBackendService_backendServiceInFlightExample(context map[string]interface{}) string {
	return acctest.Nprintf(`
...
func testAccCheckComputeBackendServiceDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(compute.Product, config), "projects/{{project}}/global/backendServices/{{name}}"))
			if err != nil {
				return err
			}
