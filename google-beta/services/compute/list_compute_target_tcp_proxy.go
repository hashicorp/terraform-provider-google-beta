...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeTargetTcpProxyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeTargetTcpProxyListResource{}

type ComputeTargetTcpProxyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeTargetTcpProxyListResource() list.ListResource {
	listR := &ComputeTargetTcpProxyListResource{}
	listR.TypeName = "google_compute_target_tcp_proxy"
	listR.SDKv2Resource = ResourceComputeTargetTcpProxy()
...
// ComputeTargetTcpProxyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeTargetTcpProxyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeTargetTcpProxyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeTargetTcpProxyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeTargetTcpProxys(
			listR.Client,
			project,
...
func ListComputeTargetTcpProxys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeTargetTcpProxy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/targetTcpProxies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeTargetTcpProxy(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeTargetTcpProxyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
