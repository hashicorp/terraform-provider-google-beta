...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeTargetInstance,
		DeleteFunction: testSweepComputeTargetInstance,
	}
...
func testSweepComputeTargetInstance(_ string) error {
	return listAndActionComputeTargetInstance(deleteResourceComputeTargetInstance)
}

func listAndActionComputeTargetInstance(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeTargetInstance"
	...
	intermediateValues[0]["zone"] = "southamerica-west1-a"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/aggregated/targetInstances", "?")[0]
...
func deleteResourceComputeTargetInstance(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/zones/{{zone}}/targetInstances/{{name}}"
	if obj["zone"] == nil {
