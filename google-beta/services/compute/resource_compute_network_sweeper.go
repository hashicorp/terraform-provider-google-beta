...
func testSweepComputeNetwork(_ string) error {
	return listAndActionComputeNetwork(deleteResourceComputeNetwork)
}

func listAndActionComputeNetwork(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeNetwork"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/networks", "?")[0]
...
func deleteResourceComputeNetwork(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/networks/{{name}}"
