...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_compute_cross_site_network",
		ListAndAction:  listAndActionComputeCrossSiteNetwork,
		DeleteFunction: testSweepComputeCrossSiteNetwork,
	}
...
func testSweepComputeCrossSiteNetwork(_ string) error {
	return listAndActionComputeCrossSiteNetwork(deleteResourceComputeCrossSiteNetwork)
}

func listAndActionComputeCrossSiteNetwork(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeCrossSiteNetwork"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/crossSiteNetworks", "?")[0]
		...
		resourceList, ok := res["crossSiteNetworks"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'crossSiteNetworks'")
		} else {
...
func deleteResourceComputeCrossSiteNetwork(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/crossSiteNetworks/{{name}}"
