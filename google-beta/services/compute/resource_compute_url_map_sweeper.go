...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeUrlMap,
		DeleteFunction: testSweepComputeUrlMap,
	}
...
func testSweepComputeUrlMap(_ string) error {
	return listAndActionComputeUrlMap(deleteResourceComputeUrlMap)
}

func listAndActionComputeUrlMap(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeUrlMap"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/urlMaps", "?")[0]
...
func deleteResourceComputeUrlMap(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/urlMaps/{{name}}"
