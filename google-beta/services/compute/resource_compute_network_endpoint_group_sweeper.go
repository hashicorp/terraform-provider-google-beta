...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeNetworkEndpointGroup,
		DeleteFunction: testSweepComputeNetworkEndpointGroup,
	}
...
func testSweepComputeNetworkEndpointGroup(_ string) error {
	return listAndActionComputeNetworkEndpointGroup(deleteResourceComputeNetworkEndpointGroup)
}

func listAndActionComputeNetworkEndpointGroup(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/aggregated/networkEndpointGroups", "?")[0]
...
func deleteResourceComputeNetworkEndpointGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/zones/{{zone}}/networkEndpointGroups/{{name}}"
	if obj["zone"] == nil {
