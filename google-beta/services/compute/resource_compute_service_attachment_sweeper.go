...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeServiceAttachment,
		DeleteFunction: testSweepComputeServiceAttachment,
	}
...
func testSweepComputeServiceAttachment(_ string) error {
	return listAndActionComputeServiceAttachment(deleteResourceComputeServiceAttachment)
}

func listAndActionComputeServiceAttachment(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[3]["region"] = "us-east1"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'serviceAttachments'")
		} else {
...
func deleteResourceComputeServiceAttachment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
