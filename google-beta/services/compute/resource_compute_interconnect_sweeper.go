...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeInterconnect,
		DeleteFunction: testSweepComputeInterconnect,
	}
...
func testSweepComputeInterconnect(_ string) error {
	return listAndActionComputeInterconnect(deleteResourceComputeInterconnect)
}

func listAndActionComputeInterconnect(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeInterconnect"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/interconnects", "?")[0]
		...
		resourceList, ok := res["interconnects"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'interconnects'")
		} else {
...
func deleteResourceComputeInterconnect(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/interconnects/{{name}}"
