...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeRegionNetworkEndpointGroupListResource,
	}.Register()
}

var _ list.ListResource = &ComputeRegionNetworkEndpointGroupListResource{}

type ComputeRegionNetworkEndpointGroupListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeRegionNetworkEndpointGroupListResource() list.ListResource {
	listR := &ComputeRegionNetworkEndpointGroupListResource{}
	listR.TypeName = "google_compute_region_network_endpoint_group"
	listR.SDKv2Resource = ResourceComputeRegionNetworkEndpointGroup()
...
// ComputeRegionNetworkEndpointGroupListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeRegionNetworkEndpointGroupListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeRegionNetworkEndpointGroupListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeRegionNetworkEndpointGroupListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeRegionNetworkEndpointGroups(
			listR.Client,
			region,
			project,
...
func ListComputeRegionNetworkEndpointGroups(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeRegionNetworkEndpointGroup().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/networkEndpointGroups")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeRegionNetworkEndpointGroup(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeRegionNetworkEndpointGroupFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
