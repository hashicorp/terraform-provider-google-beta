...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeTargetSslProxyListResource,
	}.Register()
}

var _ list.ListResource = &ComputeTargetSslProxyListResource{}

type ComputeTargetSslProxyListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeTargetSslProxyListResource() list.ListResource {
	listR := &ComputeTargetSslProxyListResource{}
	listR.TypeName = "google_compute_target_ssl_proxy"
	listR.SDKv2Resource = ResourceComputeTargetSslProxy()
...
// ComputeTargetSslProxyListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeTargetSslProxyListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeTargetSslProxyListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeTargetSslProxyListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListComputeTargetSslProxys(
			listR.Client,
			project,
...
func ListComputeTargetSslProxys(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeTargetSslProxy().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/global/targetSslProxies")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeTargetSslProxy(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeTargetSslProxyFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
