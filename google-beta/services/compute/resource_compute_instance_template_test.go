...
func TestAccComputeInstanceTemplate_partnerMetadata(t *testing.T) {
	...
	expectedPartnerMetadata := map[string]map[string]interface{}{
		namespace: {
...
func testAccCheckComputeInstanceTemplateMaintenanceInterval(instanceTemplate *map[string]interface{}, maintenance_interval string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		props, _ := (*instanceTemplate)["properties"].(map[string]interface{})
		sched, _ := props["scheduling"].(map[string]interface{})
		actual, _ := sched["maintenanceInterval"].(string)
...
func testAccCheckComputeInstanceTemplatePartnerMetadata(instanceTemplate *map[string]interface{}, expectedPartnerMetadata map[string]map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if instanceTemplate == nil {
			return fmt.Errorf("instance template is nil")
		}
		props, _ := (*instanceTemplate)["properties"].(map[string]interface{})
		pm, ok := props["partnerMetadata"].(map[string]interface{})
		if !ok {
			return fmt.Errorf("no partner metadata")
		}
		actualPartnerMetadataMap := make(map[string]interface{})
		for key, rawEntry := range pm {
			entry, _ := rawEntry.(map[string]interface{})
			actualPartnerMetadataMap[key] = entry["entries"]
		}
		...
		if !reflect.DeepEqual(actualPartnerMetadataMap, expectedPartnerMetadataMap) {
			return fmt.Errorf("got the wrong instance partne metadata action: have: %+v; want: %+v", actualPartnerMetadataMap, expectedPartnerMetadataMap)
		}
		return nil
