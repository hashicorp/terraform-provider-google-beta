...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeRegionNetworkPolicy,
		DeleteFunction: testSweepComputeRegionNetworkPolicy,
	}
...
func testSweepComputeRegionNetworkPolicy(_ string) error {
	return listAndActionComputeRegionNetworkPolicy(deleteResourceComputeRegionNetworkPolicy)
}

func listAndActionComputeRegionNetworkPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeRegionNetworkPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/networkPolicies", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'networkPolicies'")
		} else {
...
func deleteResourceComputeRegionNetworkPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/networkPolicies/{{name}}"
