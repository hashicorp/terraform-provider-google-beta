...
func TestAccComputeResourcePolicy_resourcePolicyWorkloadPolicyAcceleratorTopologyModeExample(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeResourcePolicy_resourcePolicyWorkloadPolicyAcceleratorTopologyModeExample(context),
			},
			{
				ResourceName:            "google_compute_resource_policy.bar",
				ImportState:             true,
...
func TestAccComputeResourcePolicy_resourcePolicyPlacementPolicyTpuTopologyExample(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeResourcePolicy_resourcePolicyPlacementPolicyTpuTopologyExample(context),
			},
			{
				ResourceName:            "google_compute_resource_policy.baz",
				ImportState:             true,
...
func testAccCheckComputeResourcePolicyDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(compute.Product, config), "projects/{{project}}/regions/{{region}}/resourcePolicies/{{name}}"))
			if err != nil {
				return err
			}
