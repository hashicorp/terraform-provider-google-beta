...
func TestAccComputeForwardingRule_forwardingRulePscTargetUpdateRecreate(t *testing.T) {
	t.Parallel()
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccComputeForwardingRule_forwardingRulePscTargetUpdateRecreate(context, false),
			},
			{
				ResourceName:      "google_compute_forwarding_rule.default",
				ImportState:       true,
			...
			{
				Config: testAccComputeForwardingRule_forwardingRulePscTargetUpdateRecreate(context, true),
...
func testAccComputeForwardingRule_forwardingRulePscTargetUpdateRecreate(context map[string]interface{}, useSecondTarget bool) string {
	var targetService string
	if useSecondTarget {
		targetService = "google_compute_service_attachment.producer_service_attachment_two.self_link"
	} else {
		targetService = "google_compute_service_attachment.producer_service_attachment_one.self_link"
	}
...
resource "google_compute_subnetwork" "psc_producer_nat_subnet_one" {
  name          = "tf-test-producer-psc-net-one%{random_suffix}"
  ip_cidr_range = "10.1.0.0/16"
  region        = "us-central1"
...
resource "google_compute_subnetwork" "psc_producer_nat_subnet_two" {
  name          = "tf-test-producer-psc-net-two%{random_suffix}"
...
resource "google_compute_service_attachment" "producer_service_attachment_one" {
  name        = "tf-test-producer-service-one%{random_suffix}"
  region      = "us-central1"
  description = "A service attachment configured with Terraform"

  enable_proxy_protocol = true
  connection_preference = "ACCEPT_AUTOMATIC"
  nat_subnets           = [google_compute_subnetwork.psc_producer_nat_subnet_one.name]
  target_service        = google_compute_forwarding_rule.producer_target_service_one.id
}

resource "google_compute_service_attachment" "producer_service_attachment_two" {
  name        = "tf-test-producer-service-two%{random_suffix}"
  region      = "us-central1"
  description = "A service attachment configured with Terraform"

  enable_proxy_protocol = true
  connection_preference = "ACCEPT_AUTOMATIC"
  nat_subnets           = [google_compute_subnetwork.psc_producer_nat_subnet_two.name]
  target_service        = google_compute_forwarding_rule.producer_target_service_two.id
}

resource "google_compute_forwarding_rule" "producer_target_service_one" {
  name     = "tf-test-producer-forwarding-rule-one%{random_suffix}"
  region   = "us-central1"
...
resource "google_compute_forwarding_rule" "producer_target_service_two" {
  name     = "tf-test-producer-forwarding-rule-two%{random_suffix}"
  region   = "us-central1"
