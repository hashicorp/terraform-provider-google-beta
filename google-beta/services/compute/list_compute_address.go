...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Compute",
		Func:        NewComputeAddressListResource,
	}.Register()
}

var _ list.ListResource = &ComputeAddressListResource{}

type ComputeAddressListResource struct {
	tpgresource.ListResourceMetadata
}

func NewComputeAddressListResource() list.ListResource {
	listR := &ComputeAddressListResource{}
	listR.TypeName = "google_compute_address"
	listR.SDKv2Resource = ResourceComputeAddress()
...
// ComputeAddressListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ComputeAddressListModel struct {
	Region  types.String `tfsdk:"region"`
	Project types.String `tfsdk:"project"`
}

func (listR *ComputeAddressListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ComputeAddressListModel
...
func ListComputeAddresss(config *transport_tpg.Config,
	region string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceComputeAddress().Data(&terraform.InstanceState{})
	if region != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ComputeBasePath}}projects/{{project}}/regions/{{region}}/addresses")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceComputeAddress(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceComputeAddressFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
