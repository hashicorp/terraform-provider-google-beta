...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeInterconnectGroup,
		DeleteFunction: testSweepComputeInterconnectGroup,
	}
...
func testSweepComputeInterconnectGroup(_ string) error {
	return listAndActionComputeInterconnectGroup(deleteResourceComputeInterconnectGroup)
}

func listAndActionComputeInterconnectGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeInterconnectGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/interconnectGroups", "?")[0]
		...
		resourceList, ok := res["interconnectGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'interconnectGroups'")
		} else {
...
func deleteResourceComputeInterconnectGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/interconnectGroups/{{name}}"
