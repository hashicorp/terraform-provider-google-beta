...
func testSweepComputeRouter(_ string) error {
	return listAndActionComputeRouter(deleteResourceComputeRouter)
}

func listAndActionComputeRouter(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeRouter"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/routers", "?")[0]
...
func deleteResourceComputeRouter(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/regions/{{region}}/routers/{{name}}"
