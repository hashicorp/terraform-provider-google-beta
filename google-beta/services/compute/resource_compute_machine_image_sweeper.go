...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeMachineImage,
		DeleteFunction: testSweepComputeMachineImage,
	}
...
func testSweepComputeMachineImage(_ string) error {
	return listAndActionComputeMachineImage(deleteResourceComputeMachineImage)
}

func listAndActionComputeMachineImage(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeMachineImage"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/machineImages", "?")[0]
...
func deleteResourceComputeMachineImage(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/machineImages/{{name}}"
