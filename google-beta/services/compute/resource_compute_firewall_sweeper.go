...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeFirewall,
		DeleteFunction: testSweepComputeFirewall,
	}
...
func testSweepComputeFirewall(_ string) error {
	return listAndActionComputeFirewall(deleteResourceComputeFirewall)
}

func listAndActionComputeFirewall(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeFirewall"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/firewalls", "?")[0]
...
func deleteResourceComputeFirewall(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/firewalls/{{name}}"
