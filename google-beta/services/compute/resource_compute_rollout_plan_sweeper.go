...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionComputeRolloutPlan,
		DeleteFunction: testSweepComputeRolloutPlan,
	}
...
func testSweepComputeRolloutPlan(_ string) error {
	return listAndActionComputeRolloutPlan(deleteResourceComputeRolloutPlan)
}

func listAndActionComputeRolloutPlan(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ComputeRolloutPlan"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://compute.googleapis.com/compute/beta/projects/{{project}}/global/rolloutPlans", "?")[0]
		...
		resourceList, ok := res["rolloutPlans"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'rolloutPlans'")
		} else {
...
func deleteResourceComputeRolloutPlan(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://compute.googleapis.com/compute/beta/projects/{{project}}/global/rolloutPlans/{{name}}"
