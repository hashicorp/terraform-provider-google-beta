...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDataPipelinePipeline,
		DeleteFunction: testSweepDataPipelinePipeline,
	}
...
func testSweepDataPipelinePipeline(_ string) error {
	return listAndActionDataPipelinePipeline(deleteResourceDataPipelinePipeline)
}

func listAndActionDataPipelinePipeline(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DataPipelinePipeline"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://datapipelines.googleapis.com/v1/projects/{{project}}/locations/{{region}}/pipelines", "?")[0]
		...
		resourceList, ok := res["pipelines"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'pipelines'")
		} else {
...
func deleteResourceDataPipelinePipeline(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://datapipelines.googleapis.com/v1/projects/{{project}}/locations/{{region}}/pipelines/{{name}}"
