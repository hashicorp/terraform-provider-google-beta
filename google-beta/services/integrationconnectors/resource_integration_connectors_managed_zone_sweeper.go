...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_integration_connectors_managed_zone",
		ListAndAction:  listAndActionIntegrationConnectorsManagedZone,
		DeleteFunction: testSweepIntegrationConnectorsManagedZone,
	}
...
func testSweepIntegrationConnectorsManagedZone(_ string) error {
	return listAndActionIntegrationConnectorsManagedZone(deleteResourceIntegrationConnectorsManagedZone)
}

func listAndActionIntegrationConnectorsManagedZone(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "IntegrationConnectorsManagedZone"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://connectors.googleapis.com/v1/projects/{{project}}/locations/global/managedZones", "?")[0]
		...
		resourceList, ok := res["managedZones"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'managedZones'")
		} else {
...
func deleteResourceIntegrationConnectorsManagedZone(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://connectors.googleapis.com/v1/projects/{{project}}/locations/global/managedZones/{{name}}"
