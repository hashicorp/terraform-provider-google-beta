...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_integration_connectors_endpoint_attachment",
		ListAndAction:  listAndActionIntegrationConnectorsEndpointAttachment,
		DeleteFunction: testSweepIntegrationConnectorsEndpointAttachment,
	}
...
func testSweepIntegrationConnectorsEndpointAttachment(_ string) error {
	return listAndActionIntegrationConnectorsEndpointAttachment(deleteResourceIntegrationConnectorsEndpointAttachment)
}

func listAndActionIntegrationConnectorsEndpointAttachment(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "IntegrationConnectorsEndpointAttachment"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://connectors.googleapis.com/v1/projects/{{project}}/locations/{{location}}/endpointAttachments", "?")[0]
		...
		resourceList, ok := res["endpointAttachments"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'endpointAttachments'")
		} else {
...
func deleteResourceIntegrationConnectorsEndpointAttachment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://connectors.googleapis.com/v1/projects/{{project}}/locations/{{location}}/endpointAttachments/{{name}}"
