...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionGKEHub2RolloutSequence,
		DeleteFunction: testSweepGKEHub2RolloutSequence,
	}
...
func testSweepGKEHub2RolloutSequence(_ string) error {
	return listAndActionGKEHub2RolloutSequence(deleteResourceGKEHub2RolloutSequence)
}

func listAndActionGKEHub2RolloutSequence(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GKEHub2RolloutSequence"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://gkehub.googleapis.com/v1beta/projects/{{project}}/locations/global/rolloutSequences", "?")[0]
		...
		resourceList, ok := res["rolloutSequences"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'rolloutSequences'")
		} else {
...
func deleteResourceGKEHub2RolloutSequence(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://gkehub.googleapis.com/v1beta/projects/{{project}}/locations/global/rolloutSequences/{{rollout_sequence_id}}"
