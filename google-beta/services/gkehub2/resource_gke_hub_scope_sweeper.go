...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionGKEHub2Scope,
		DeleteFunction: testSweepGKEHub2Scope,
	}
...
func testSweepGKEHub2Scope(_ string) error {
	return listAndActionGKEHub2Scope(deleteResourceGKEHub2Scope)
}

func listAndActionGKEHub2Scope(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "GKEHub2Scope"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://gkehub.googleapis.com/v1beta/projects/{{project}}/locations/global/scopes", "?")[0]
		...
		resourceList, ok := res["scopes"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'scopes'")
		} else {
...
func deleteResourceGKEHub2Scope(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://gkehub.googleapis.com/v1beta/projects/{{project}}/locations/global/scopes/{{scope_id}}"
