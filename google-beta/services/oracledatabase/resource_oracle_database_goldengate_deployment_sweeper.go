...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionOracleDatabaseGoldengateDeployment,
		DeleteFunction: testSweepOracleDatabaseGoldengateDeployment,
	}
...
func testSweepOracleDatabaseGoldengateDeployment(_ string) error {
	return listAndActionOracleDatabaseGoldengateDeployment(deleteResourceOracleDatabaseGoldengateDeployment)
}

func listAndActionOracleDatabaseGoldengateDeployment(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "OracleDatabaseGoldengateDeployment"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://oracledatabase.googleapis.com/v1/projects/{{project}}/locations/{{location}}/goldengateDeployments", "?")[0]
		...
		resourceList, ok := res["goldengateDeployments"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'goldengateDeployments'")
		} else {
...
func deleteResourceOracleDatabaseGoldengateDeployment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://oracledatabase.googleapis.com/v1/projects/{{project}}/locations/{{location}}/goldengateDeployments/{{goldengate_deployment_id}}"
