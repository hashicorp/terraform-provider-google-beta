...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_firebase_app_hosting_backend",
		ListAndAction:  listAndActionFirebaseAppHostingBackend,
		DeleteFunction: testSweepFirebaseAppHostingBackend,
	}
...
func testSweepFirebaseAppHostingBackend(_ string) error {
	return listAndActionFirebaseAppHostingBackend(deleteResourceFirebaseAppHostingBackend)
}

func listAndActionFirebaseAppHostingBackend(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "FirebaseAppHostingBackend"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://firebaseapphosting.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/backends", "?")[0]
		...
		resourceList, ok := res["backends"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'backends'")
		} else {
...
func deleteResourceFirebaseAppHostingBackend(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://firebaseapphosting.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/backends/{{backend_id}}?force=true"
