...
func testSweepVmwareengineNetworkPeering(_ string) error {
	return listAndActionVmwareengineNetworkPeering(deleteResourceVmwareengineNetworkPeering)
}

func listAndActionVmwareengineNetworkPeering(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'networkPeerings'")
		} else {
...
func deleteResourceVmwareengineNetworkPeering(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
