...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionVmwareengineNetwork,
		DeleteFunction: testSweepVmwareengineNetwork,
	}
	// Add dependencies
...
func testSweepVmwareengineNetwork(_ string) error {
	return listAndActionVmwareengineNetwork(deleteResourceVmwareengineNetwork)
}

func listAndActionVmwareengineNetwork(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[2]["region"] = "me-west1"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'vmwareEngineNetworks'")
		} else {
...
func deleteResourceVmwareengineNetwork(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
