...
func TestAccVmwareengineNetworkPolicy_vmwareEngineNetworkPolicyBasicExample(t *testing.T) {
	...
	context := map[string]interface{}{
		...
		"network_name":        "tf-test-sample-network" + randomSuffix,
		"network_policy_name": "tf-test-sample-network-policy" + randomSuffix,
...
func testAccCheckVmwareengineNetworkPolicyDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(vmwareengine.Product, config), "projects/{{project}}/locations/{{location}}/networkPolicies/{{name}}"))
			if err != nil {
				return err
			}
