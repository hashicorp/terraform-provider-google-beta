...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDatabaseMigrationServicePrivateConnection,
		DeleteFunction: testSweepDatabaseMigrationServicePrivateConnection,
	}
...
func testSweepDatabaseMigrationServicePrivateConnection(_ string) error {
	return listAndActionDatabaseMigrationServicePrivateConnection(deleteResourceDatabaseMigrationServicePrivateConnection)
}

func listAndActionDatabaseMigrationServicePrivateConnection(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DatabaseMigrationServicePrivateConnection"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://datamigration.googleapis.com/v1/projects/{{project}}/locations/{{location}}/privateConnections", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'privateConnections'")
		} else {
...
func deleteResourceDatabaseMigrationServicePrivateConnection(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://datamigration.googleapis.com/v1/projects/{{project}}/locations/{{location}}/privateConnections/{{private_connection_id}}"
