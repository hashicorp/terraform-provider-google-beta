...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMonitoringGroup,
		DeleteFunction: testSweepMonitoringGroup,
	}
...
func testSweepMonitoringGroup(_ string) error {
	return listAndActionMonitoringGroup(deleteResourceMonitoringGroup)
}

func listAndActionMonitoringGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MonitoringGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://monitoring.googleapis.com/v3/projects/{{project}}/groups", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'groups'")
		} else {
...
func deleteResourceMonitoringGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
