...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMonitoringGenericService,
		DeleteFunction: testSweepMonitoringGenericService,
	}
...
func testSweepMonitoringGenericService(_ string) error {
	return listAndActionMonitoringGenericService(deleteResourceMonitoringGenericService)
}

func listAndActionMonitoringGenericService(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MonitoringGenericService"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://monitoring.googleapis.com/v3/projects/{{project}}/services", "?")[0]
		...
		resourceList, ok := res["genericServices"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'genericServices'")
		} else {
...
func deleteResourceMonitoringGenericService(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://monitoring.googleapis.com/v3/projects/{{project}}/services/{{service_id}}"
