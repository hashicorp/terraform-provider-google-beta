...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMonitoringMetricDescriptor,
		DeleteFunction: testSweepMonitoringMetricDescriptor,
	}
...
func testSweepMonitoringMetricDescriptor(_ string) error {
	return listAndActionMonitoringMetricDescriptor(deleteResourceMonitoringMetricDescriptor)
}

func listAndActionMonitoringMetricDescriptor(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "MonitoringMetricDescriptor"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://monitoring.googleapis.com/v3/projects/{{project}}/metricDescriptors", "?")[0]
		...
		resourceList, ok := res["metricDescriptors"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'metricDescriptors'")
		} else {
...
func deleteResourceMonitoringMetricDescriptor(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
