...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionMonitoringAlertPolicy,
		DeleteFunction: testSweepMonitoringAlertPolicy,
	}
...
func testSweepMonitoringAlertPolicy(_ string) error {
	return listAndActionMonitoringAlertPolicy(deleteResourceMonitoringAlertPolicy)
}

func listAndActionMonitoringAlertPolicy(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'alertPolicies'")
		} else {
...
func deleteResourceMonitoringAlertPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
