...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_model_armor_template",
		ListAndAction:  listAndActionModelArmorTemplate,
		DeleteFunction: testSweepModelArmorTemplate,
	}
...
func testSweepModelArmorTemplate(_ string) error {
	return listAndActionModelArmorTemplate(deleteResourceModelArmorTemplate)
}

func listAndActionModelArmorTemplate(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ModelArmorTemplate"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["templates"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'templates'")
		} else {
...
func deleteResourceModelArmorTemplate(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
