...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Apigee",
		Func:        NewApigeeDatastoreListResource,
	}.Register()
}

var _ list.ListResource = &ApigeeDatastoreListResource{}

type ApigeeDatastoreListResource struct {
	tpgresource.ListResourceMetadata
}

func NewApigeeDatastoreListResource() list.ListResource {
	listR := &ApigeeDatastoreListResource{}
	listR.TypeName = "google_apigee_datastore"
	listR.SDKv2Resource = ResourceApigeeDatastore()
...
// ApigeeDatastoreListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ApigeeDatastoreListModel struct {
	OrgId types.String `tfsdk:"org_id"`
}

func (listR *ApigeeDatastoreListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ApigeeDatastoreListModel
...
func ListApigeeDatastores(config *transport_tpg.Config,
	orgId string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceApigeeDatastore().Data(&terraform.InstanceState{})
	if orgId != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ApigeeBasePath}}{{org_id}}/analytics/datastores")
	if err != nil {
		return err
	}
	billingProject := ""
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceApigeeDatastore(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			var err error
			res, err = resourceApigeeDatastoreDecoder(d, config, res)
			if err != nil {
				return err
			}
			if res == nil {
				return fmt.Errorf("error decoding ApigeeDatastore from list response")
			}
