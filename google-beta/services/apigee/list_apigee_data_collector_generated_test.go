		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_data_collector.apigee_data_collector",
						[]string{
							"name",
							"data_collector_id",
						},
					...
					listScope.Capture(map[string]string{
						"org_id": "google_apigee_data_collector.apigee_data_collector",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeDataCollector_apigeeDataCollectorBasicTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_data_collector.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_data_collector.list_query",
...
func testAccApigeeDataCollector_apigeeDataCollectorBasicTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "org_id" { type = string }
list "google_apigee_data_collector" "list_query" {
    provider = google
	limit = 10000
    config {
        org_id = var.org_id
