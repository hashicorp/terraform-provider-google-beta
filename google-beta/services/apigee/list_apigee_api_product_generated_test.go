...
func TestAccApigeeApiProductListQuery_generated(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_api_product.apigee_api_product",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"org_id": "google_apigee_api_product.apigee_api_product",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeApiProduct_apigeeApiProductBasicTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_api_product.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_api_product.list_query",
...
func testAccApigeeApiProduct_apigeeApiProductBasicTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "org_id" { type = string }
list "google_apigee_api_product" "list_query" {
    provider = google
	limit = 10000
    config {
        org_id = var.org_id
