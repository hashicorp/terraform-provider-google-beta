		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_apigee_datastore.apigee_datastore",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"org_id": "google_apigee_datastore.apigee_datastore",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccApigeeDatastore_apigeeDatastoreBasicTestExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_apigee_datastore.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_apigee_datastore.list_query",
...
func testAccApigeeDatastore_apigeeDatastoreBasicTestExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "org_id" { type = string }
list "google_apigee_datastore" "list_query" {
    provider = google
	limit = 10000
    config {
        org_id = var.org_id
