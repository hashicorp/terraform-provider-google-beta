...
//
//	Source file: https://github.com/GoogleCloudPlatform/magic-modules/tree/main/mmv1/third_party/terraform/services/deploymentmanager/resource_deployment_manager_deployment_test.go
//
...
func testAccCheckDeploymentManagerDeploymentDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(deploymentmanager.Product, config)+"projects/{{project}}/global/deployments/{{name}}")
			if err != nil {
				return err
			}
