...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPrivatecaCaPool,
		DeleteFunction: testSweepPrivatecaCaPool,
	}
...
func testSweepPrivatecaCaPool(_ string) error {
	return listAndActionPrivatecaCaPool(deleteResourcePrivatecaCaPool)
}

func listAndActionPrivatecaCaPool(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "PrivatecaCaPool"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://privateca.googleapis.com/v1/projects/{{project}}/locations/{{location}}/caPools", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'caPools'")
		} else {
...
func deleteResourcePrivatecaCaPool(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://privateca.googleapis.com/v1/projects/{{project}}/locations/{{location}}/caPools/{{name}}"
