...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPrivatecaCertificateTemplate,
		DeleteFunction: testSweepPrivatecaCertificateTemplate,
	}
...
func testSweepPrivatecaCertificateTemplate(_ string) error {
	return listAndActionPrivatecaCertificateTemplate(deleteResourcePrivatecaCertificateTemplate)
}

func listAndActionPrivatecaCertificateTemplate(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "PrivatecaCertificateTemplate"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://privateca.googleapis.com/v1/projects/{{project}}/locations/{{location}}/certificateTemplates", "?")[0]
		...
		resourceList, ok := res["certificateTemplates"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'certificateTemplates'")
		} else {
...
func deleteResourcePrivatecaCertificateTemplate(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://privateca.googleapis.com/v1/projects/{{project}}/locations/{{location}}/certificateTemplates/{{name}}"
