...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_firebase_ai_logic_prompt_template",
		ListAndAction:  listAndActionFirebaseAILogicPromptTemplate,
		DeleteFunction: testSweepFirebaseAILogicPromptTemplate,
	}
...
func testSweepFirebaseAILogicPromptTemplate(_ string) error {
	return listAndActionFirebaseAILogicPromptTemplate(deleteResourceFirebaseAILogicPromptTemplate)
}

func listAndActionFirebaseAILogicPromptTemplate(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "FirebaseAILogicPromptTemplate"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://firebasevertexai.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/templates", "?")[0]
		...
		resourceList, ok := res["promptTemplates"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'promptTemplates'")
		} else {
...
func deleteResourceFirebaseAILogicPromptTemplate(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://firebasevertexai.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/templates/{{template_id}}"
