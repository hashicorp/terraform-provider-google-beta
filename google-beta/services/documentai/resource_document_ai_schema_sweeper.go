...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_document_ai_schema",
		ListAndAction:  listAndActionDocumentAISchema,
		DeleteFunction: testSweepDocumentAISchema,
	}
...
func testSweepDocumentAISchema(_ string) error {
	return listAndActionDocumentAISchema(deleteResourceDocumentAISchema)
}

func listAndActionDocumentAISchema(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "DocumentAISchema"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/schemas", "?")[0]
		...
		resourceList, ok := res["schemas"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'schemas'")
		} else {
...
func deleteResourceDocumentAISchema(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{location}}-documentai.googleapis.com/v1/projects/{{project}}/locations/{{location}}/schemas/{{name}}"
