...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionDocumentAIProcessor,
		DeleteFunction: testSweepDocumentAIProcessor,
	}
...
func testSweepDocumentAIProcessor(_ string) error {
	return listAndActionDocumentAIProcessor(deleteResourceDocumentAIProcessor)
}

func listAndActionDocumentAIProcessor(action sweeper.ResourceAction) error {
	var lastError error
	...
	intermediateValues[1]["region"] = "eu"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["processors"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'processors'")
		} else {
...
func deleteResourceDocumentAIProcessor(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
