...
func testAccCheckApiGatewayGatewayDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(apigateway.Product, config), "projects/{{project}}/locations/{{region}}/gateways/{{gateway_id}}"))
			if err != nil {
				return err
			}
