...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionApiGatewayGateway,
		DeleteFunction: testSweepApiGatewayGateway,
	}
...
func testSweepApiGatewayGateway(_ string) error {
	return listAndActionApiGatewayGateway(deleteResourceApiGatewayGateway)
}

func listAndActionApiGatewayGateway(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ApiGatewayGateway"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://apigateway.googleapis.com/v1beta/projects/{{project}}/locations/{{region}}/gateways", "?")[0]
		...
		resourceList, ok := res["gateways"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'gateways'")
		} else {
...
func deleteResourceApiGatewayGateway(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://apigateway.googleapis.com/v1beta/projects/{{project}}/locations/{{region}}/gateways/{{gateway_id}}"
