...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionApiGatewayApiConfig,
		DeleteFunction: testSweepApiGatewayApiConfig,
	}
	// Add parent relationship
	s.Parents = []string{"google_api_gateway_api"}

	// Register the sweeper
	sweeper.AddTestSweepers(s)
}

func testSweepApiGatewayApiConfig(_ string) error {
	return listAndActionApiGatewayApiConfig(deleteResourceApiGatewayApiConfig)
}

func listAndActionApiGatewayApiConfig(action sweeper.ResourceAction) error {
	var lastError error
	...
	parentType := "google_api_gateway_api"
	...
	if err := parentSweeper.ListAndAction(collectParentConfigApiGatewayApiConfig(&configs)); err != nil {
	...
	for _, mockConfig := range configs {
		...
		parentValue := ""
		if v, ok := mockConfig.FieldsInSchema["api"]; ok {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'apiConfigs'")
		} else {
...
func deleteResourceApiGatewayApiConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
...
// collectParentConfigApiGatewayApiConfig returns a function that collects parent configurations
func collectParentConfigApiGatewayApiConfig(configs *[]*tpgresource.ResourceDataMock) sweeper.ResourceAction {
	return func(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, parentObj map[string]interface{}) error {
		...
		if parentObj["name"] == nil {
			log.Printf("[INFO][SWEEPER_LOG] Parent google_api_gateway_api field name was nil, skipping")
			return nil
		}
		...
		childConfig.FieldsInSchema["api"] = parentValue
