...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionApiGatewayApi,
		DeleteFunction: testSweepApiGatewayApi,
	}
...
func testSweepApiGatewayApi(_ string) error {
	return listAndActionApiGatewayApi(deleteResourceApiGatewayApi)
}

func listAndActionApiGatewayApi(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'apis'")
		} else {
...
func deleteResourceApiGatewayApi(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
