...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionFirebaseStorageBucket,
		DeleteFunction: testSweepFirebaseStorageBucket,
	}
...
func testSweepFirebaseStorageBucket(_ string) error {
	return listAndActionFirebaseStorageBucket(deleteResourceFirebaseStorageBucket)
}

func listAndActionFirebaseStorageBucket(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "FirebaseStorageBucket"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://firebasestorage.googleapis.com/v1beta/projects/{{project}}/buckets", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'buckets'")
		} else {
...
func deleteResourceFirebaseStorageBucket(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://firebasestorage.googleapis.com/v1beta/projects/{{project}}/buckets/{{bucket_id}}:removeFirebase"
