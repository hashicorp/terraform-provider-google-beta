...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionFirebaseStorageDefaultBucket,
		DeleteFunction: testSweepFirebaseStorageDefaultBucket,
	}
...
func testSweepFirebaseStorageDefaultBucket(_ string) error {
	return listAndActionFirebaseStorageDefaultBucket(deleteResourceFirebaseStorageDefaultBucket)
}

func listAndActionFirebaseStorageDefaultBucket(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "FirebaseStorageDefaultBucket"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://firebasestorage.googleapis.com/v1beta/projects/{{project}}/defaultBucket", "?")[0]
		...
		resourceList, ok := res["defaultBuckets"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'defaultBuckets'")
		} else {
...
func deleteResourceFirebaseStorageDefaultBucket(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://firebasestorage.googleapis.com/v1beta/projects/{{project}}/defaultBucket"
