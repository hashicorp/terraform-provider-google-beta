...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPubsubLiteTopic,
		DeleteFunction: testSweepPubsubLiteTopic,
	}
...
func testSweepPubsubLiteTopic(_ string) error {
	return listAndActionPubsubLiteTopic(deleteResourcePubsubLiteTopic)
}

func listAndActionPubsubLiteTopic(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "PubsubLiteTopic"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-pubsublite.googleapis.com/v1/admin/projects/{{project}}/locations/{{zone}}/topics", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'topics'")
		} else {
...
func deleteResourcePubsubLiteTopic(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-pubsublite.googleapis.com/v1/admin/projects/{{project}}/locations/{{zone}}/topics/{{name}}"
