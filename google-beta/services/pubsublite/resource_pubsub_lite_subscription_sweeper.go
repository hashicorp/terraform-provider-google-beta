...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionPubsubLiteSubscription,
		DeleteFunction: testSweepPubsubLiteSubscription,
	}
...
func testSweepPubsubLiteSubscription(_ string) error {
	return listAndActionPubsubLiteSubscription(deleteResourcePubsubLiteSubscription)
}

func listAndActionPubsubLiteSubscription(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "PubsubLiteSubscription"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://{{region}}-pubsublite.googleapis.com/v1/admin/projects/{{project}}/locations/{{zone}}/subscriptions", "?")[0]
...
func deleteResourcePubsubLiteSubscription(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://{{region}}-pubsublite.googleapis.com/v1/admin/projects/{{project}}/locations/{{zone}}/subscriptions/{{name}}"
