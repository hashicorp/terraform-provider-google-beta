...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionAgentRegistryBinding,
		DeleteFunction: testSweepAgentRegistryBinding,
	}
...
func testSweepAgentRegistryBinding(_ string) error {
	return listAndActionAgentRegistryBinding(deleteResourceAgentRegistryBinding)
}

func listAndActionAgentRegistryBinding(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "AgentRegistryBinding"
	...
	for _, mockConfig := range configs {
		...
		resourceList, ok := res["bindings"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'bindings'")
		} else {
...
func deleteResourceAgentRegistryBinding(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
