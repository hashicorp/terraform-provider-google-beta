...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionRedisClusterAclPolicy,
		DeleteFunction: testSweepRedisClusterAclPolicy,
	}
...
func testSweepRedisClusterAclPolicy(_ string) error {
	return listAndActionRedisClusterAclPolicy(deleteResourceRedisClusterAclPolicy)
}

func listAndActionRedisClusterAclPolicy(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "RedisClusterAclPolicy"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://redis.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/aclPolicies", "?")[0]
		...
		resourceList, ok := res["clusterAclPolicies"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'clusterAclPolicies'")
		} else {
...
func deleteResourceRedisClusterAclPolicy(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://redis.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/aclPolicies/{{acl_policy_id}}"
