...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionRedisInstance,
		DeleteFunction: testSweepRedisInstance,
	}
...
func testSweepRedisInstance(_ string) error {
	return listAndActionRedisInstance(deleteResourceRedisInstance)
}

func listAndActionRedisInstance(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "RedisInstance"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://redis.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/instances", "?")[0]
...
func deleteResourceRedisInstance(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://redis.googleapis.com/v1beta1/projects/{{project}}/locations/{{region}}/instances/{{name}}"
