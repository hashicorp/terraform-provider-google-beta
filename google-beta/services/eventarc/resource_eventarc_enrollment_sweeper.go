...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEventarcEnrollment,
		DeleteFunction: testSweepEventarcEnrollment,
	}
...
func testSweepEventarcEnrollment(_ string) error {
	return listAndActionEventarcEnrollment(deleteResourceEventarcEnrollment)
}

func listAndActionEventarcEnrollment(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "EventarcEnrollment"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/enrollments", "?")[0]
		...
		resourceList, ok := res["enrollments"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'enrollments'")
		} else {
...
func deleteResourceEventarcEnrollment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/enrollments/{{enrollment_id}}"
