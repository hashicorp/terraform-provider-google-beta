...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEventarcGoogleApiSource,
		DeleteFunction: testSweepEventarcGoogleApiSource,
	}
...
func testSweepEventarcGoogleApiSource(_ string) error {
	return listAndActionEventarcGoogleApiSource(deleteResourceEventarcGoogleApiSource)
}

func listAndActionEventarcGoogleApiSource(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "EventarcGoogleApiSource"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/googleApiSources", "?")[0]
		...
		resourceList, ok := res["googleApiSources"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'googleApiSources'")
		} else {
...
func deleteResourceEventarcGoogleApiSource(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/googleApiSources/{{google_api_source_id}}"
