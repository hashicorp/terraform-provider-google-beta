...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEventarcMessageBus,
		DeleteFunction: testSweepEventarcMessageBus,
	}
...
func testSweepEventarcMessageBus(_ string) error {
	return listAndActionEventarcMessageBus(deleteResourceEventarcMessageBus)
}

func listAndActionEventarcMessageBus(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "EventarcMessageBus"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/messageBuses", "?")[0]
...
func deleteResourceEventarcMessageBus(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/messageBuses/{{message_bus_id}}"
