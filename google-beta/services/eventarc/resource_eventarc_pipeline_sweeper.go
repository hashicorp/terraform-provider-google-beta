...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEventarcPipeline,
		DeleteFunction: testSweepEventarcPipeline,
	}
...
func testSweepEventarcPipeline(_ string) error {
	return listAndActionEventarcPipeline(deleteResourceEventarcPipeline)
}

func listAndActionEventarcPipeline(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "EventarcPipeline"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/pipelines", "?")[0]
...
func deleteResourceEventarcPipeline(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/pipelines/{{pipeline_id}}"
