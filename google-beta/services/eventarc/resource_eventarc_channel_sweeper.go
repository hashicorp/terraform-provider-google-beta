...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionEventarcChannel,
		DeleteFunction: testSweepEventarcChannel,
	}
...
func testSweepEventarcChannel(_ string) error {
	return listAndActionEventarcChannel(deleteResourceEventarcChannel)
}

func listAndActionEventarcChannel(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "EventarcChannel"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/channels", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'channels'")
		} else {
...
func deleteResourceEventarcChannel(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://eventarc.googleapis.com/v1/projects/{{project}}/locations/{{location}}/channels/{{name}}"
