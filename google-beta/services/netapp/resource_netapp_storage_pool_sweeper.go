...
func init() {
	sweeper.AddTestSweepersLegacy("NetappStoragePool", testSweepNetappStoragePool)
}
...
func testSweepNetappStoragePool(region string) error {
	...
	for _, r := range regions {
		...
		listTemplate := strings.Split("https://netapp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/storagePools", "?")[0]
		...
		for _, ri := range rl {
			...
			deleteTemplate := "https://netapp.googleapis.com/v1/projects/{{project}}/locations/{{location}}/storagePools/{{name}}"
