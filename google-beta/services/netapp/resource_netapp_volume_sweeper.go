...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetappVolume,
		DeleteFunction: testSweepNetappVolume,
	}
...
func testSweepNetappVolume(_ string) error {
	return listAndActionNetappVolume(deleteResourceNetappVolume)
}

func listAndActionNetappVolume(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetappVolume"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://netapp.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/volumes", "?")[0]
		...
		resourceList, ok := res["volumes"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'volumes'")
		} else {
...
func deleteResourceNetappVolume(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://netapp.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/volumes/{{name}}"
