...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSaasRuntimeUnit,
		DeleteFunction: testSweepSaasRuntimeUnit,
	}
...
func testSweepSaasRuntimeUnit(_ string) error {
	return listAndActionSaasRuntimeUnit(deleteResourceSaasRuntimeUnit)
}

func listAndActionSaasRuntimeUnit(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasRuntimeUnit"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/units", "?")[0]
		...
		resourceList, ok := res["units"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'units'")
		} else {
...
func deleteResourceSaasRuntimeUnit(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/units/{{unit_id}}"
