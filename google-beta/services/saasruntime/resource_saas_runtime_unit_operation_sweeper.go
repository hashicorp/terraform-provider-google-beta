...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSaasRuntimeUnitOperation,
		DeleteFunction: testSweepSaasRuntimeUnitOperation,
	}
...
func testSweepSaasRuntimeUnitOperation(_ string) error {
	return listAndActionSaasRuntimeUnitOperation(deleteResourceSaasRuntimeUnitOperation)
}

func listAndActionSaasRuntimeUnitOperation(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasRuntimeUnitOperation"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/unitOperations", "?")[0]
		...
		resourceList, ok := res["unitOperations"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'unitOperations'")
		} else {
...
func deleteResourceSaasRuntimeUnitOperation(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/unitOperations/{{unit_operation_id}}"
