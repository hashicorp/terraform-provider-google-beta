...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSaasRuntimeRolloutKind,
		DeleteFunction: testSweepSaasRuntimeRolloutKind,
	}
...
func testSweepSaasRuntimeRolloutKind(_ string) error {
	return listAndActionSaasRuntimeRolloutKind(deleteResourceSaasRuntimeRolloutKind)
}

func listAndActionSaasRuntimeRolloutKind(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasRuntimeRolloutKind"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/rolloutKinds", "?")[0]
		...
		resourceList, ok := res["rolloutKinds"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'rolloutKinds'")
		} else {
...
func deleteResourceSaasRuntimeRolloutKind(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/rolloutKinds/{{rollout_kind_id}}"
