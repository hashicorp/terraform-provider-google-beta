...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSaasRuntimeSaas,
		DeleteFunction: testSweepSaasRuntimeSaas,
	}
...
func testSweepSaasRuntimeSaas(_ string) error {
	return listAndActionSaasRuntimeSaas(deleteResourceSaasRuntimeSaas)
}

func listAndActionSaasRuntimeSaas(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasRuntimeSaas"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/saas", "?")[0]
		...
		resourceList, ok := res["saases"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'saases'")
		} else {
...
func deleteResourceSaasRuntimeSaas(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/saas/{{saas_id}}"
