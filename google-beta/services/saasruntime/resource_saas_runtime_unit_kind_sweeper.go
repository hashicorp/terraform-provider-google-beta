...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSaasRuntimeUnitKind,
		DeleteFunction: testSweepSaasRuntimeUnitKind,
	}
...
func testSweepSaasRuntimeUnitKind(_ string) error {
	return listAndActionSaasRuntimeUnitKind(deleteResourceSaasRuntimeUnitKind)
}

func listAndActionSaasRuntimeUnitKind(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasRuntimeUnitKind"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/unitKinds", "?")[0]
		...
		resourceList, ok := res["unitKinds"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'unitKinds'")
		} else {
...
func deleteResourceSaasRuntimeUnitKind(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/unitKinds/{{unit_kind_id}}"
