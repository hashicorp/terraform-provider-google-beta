...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSaasRuntimeTenant,
		DeleteFunction: testSweepSaasRuntimeTenant,
	}
...
func testSweepSaasRuntimeTenant(_ string) error {
	return listAndActionSaasRuntimeTenant(deleteResourceSaasRuntimeTenant)
}

func listAndActionSaasRuntimeTenant(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasRuntimeTenant"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/tenants", "?")[0]
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'tenants'")
		} else {
...
func deleteResourceSaasRuntimeTenant(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/tenants/{{tenant_id}}"
