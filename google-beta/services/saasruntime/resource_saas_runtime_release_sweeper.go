...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSaasRuntimeRelease,
		DeleteFunction: testSweepSaasRuntimeRelease,
	}
...
func testSweepSaasRuntimeRelease(_ string) error {
	return listAndActionSaasRuntimeRelease(deleteResourceSaasRuntimeRelease)
}

func listAndActionSaasRuntimeRelease(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "SaasRuntimeRelease"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/releases", "?")[0]
		...
		resourceList, ok := res["releases"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'releases'")
		} else {
...
func deleteResourceSaasRuntimeRelease(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://saasservicemgmt.googleapis.com/v1beta1/projects/{{project}}/locations/{{location}}/releases/{{release_id}}"
