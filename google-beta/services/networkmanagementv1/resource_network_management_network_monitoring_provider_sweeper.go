...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionNetworkManagementv1NetworkMonitoringProvider,
		DeleteFunction: testSweepNetworkManagementv1NetworkMonitoringProvider,
	}
...
func testSweepNetworkManagementv1NetworkMonitoringProvider(_ string) error {
	return listAndActionNetworkManagementv1NetworkMonitoringProvider(deleteResourceNetworkManagementv1NetworkMonitoringProvider)
}

func listAndActionNetworkManagementv1NetworkMonitoringProvider(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "NetworkManagementv1NetworkMonitoringProvider"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://networkmanagement.googleapis.com/v1/projects/{{project}}/locations/{{location}}/networkMonitoringProviders", "?")[0]
		...
		resourceList, ok := res["networkMonitoringProviders"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'networkMonitoringProviders'")
		} else {
...
func deleteResourceNetworkManagementv1NetworkMonitoringProvider(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://networkmanagement.googleapis.com/v1/projects/{{project}}/locations/{{location}}/networkMonitoringProviders/{{network_monitoring_provider_id}}"
