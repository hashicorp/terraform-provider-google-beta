...
func init() {
	registry.FrameworkListResource{
		...
		Func:        NewSecretManagerSecretListResource,
	}.Register()
}

var _ list.ListResource = &SecretManagerSecretListResource{}

type SecretManagerSecretListResource struct {
	tpgresource.ListResourceMetadata
}

func NewSecretManagerSecretListResource() list.ListResource {
	listR := &SecretManagerSecretListResource{}
	listR.TypeName = "google_secret_manager_secret"
	listR.SDKv2Resource = ResourceSecretManagerSecret()
...
// SecretManagerSecretListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type SecretManagerSecretListModel struct {
	Project types.String `tfsdk:"project"`
}

func (listR *SecretManagerSecretListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data SecretManagerSecretListModel
	...
	stream.Results = func(push func(list.ListResult) bool) {
		err := ListSecretManagerSecrets(
			listR.Client,
			project,
			func(rd *schema.ResourceData) error {
				result := listReq.NewListResult(ctx)

				if err := listR.SetResult(ctx, listReq.IncludeResource, &result, rd, "name", "secret_id"); err != nil {
					return err
				}
...
func ListSecretManagerSecrets(config *transport_tpg.Config,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceSecretManagerSecret().Data(&terraform.InstanceState{})
	if project != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{SecretManagerBasePath}}projects/{{project}}/secrets")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceSecretManagerSecret(),
		ListURL:        url,
		...
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if err = ResourceSecretManagerSecretFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
