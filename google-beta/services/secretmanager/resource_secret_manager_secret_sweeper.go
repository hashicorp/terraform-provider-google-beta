...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionSecretManagerSecret,
		DeleteFunction: testSweepSecretManagerSecret,
	}
...
func testSweepSecretManagerSecret(_ string) error {
	return listAndActionSecretManagerSecret(deleteResourceSecretManagerSecret)
}

func listAndActionSecretManagerSecret(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'secrets'")
		} else {
...
func deleteResourceSecretManagerSecret(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
