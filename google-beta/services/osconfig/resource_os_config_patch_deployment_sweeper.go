...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionOSConfigPatchDeployment,
		DeleteFunction: testSweepOSConfigPatchDeployment,
	}
...
func testSweepOSConfigPatchDeployment(_ string) error {
	return listAndActionOSConfigPatchDeployment(deleteResourceOSConfigPatchDeployment)
}

func listAndActionOSConfigPatchDeployment(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'patchDeployments'")
		} else {
...
func deleteResourceOSConfigPatchDeployment(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
