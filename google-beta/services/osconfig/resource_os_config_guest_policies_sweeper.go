...
func init() {
	...
	s := &sweeper.Sweeper{
		Name:           "google_os_config_guest_policies",
		ListAndAction:  listAndActionOSConfigGuestPolicies,
		DeleteFunction: testSweepOSConfigGuestPolicies,
	}
...
func testSweepOSConfigGuestPolicies(_ string) error {
	return listAndActionOSConfigGuestPolicies(deleteResourceOSConfigGuestPolicies)
}

func listAndActionOSConfigGuestPolicies(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'guestPolicies'")
		} else {
...
func deleteResourceOSConfigGuestPolicies(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
