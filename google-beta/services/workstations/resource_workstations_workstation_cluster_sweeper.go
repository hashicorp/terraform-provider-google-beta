...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionWorkstationsWorkstationCluster,
		DeleteFunction: testSweepWorkstationsWorkstationCluster,
	}
...
func testSweepWorkstationsWorkstationCluster(_ string) error {
	return listAndActionWorkstationsWorkstationCluster(deleteResourceWorkstationsWorkstationCluster)
}

func listAndActionWorkstationsWorkstationCluster(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "WorkstationsWorkstationCluster"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://workstations.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/workstationClusters", "?")[0]
		...
		resourceList, ok := res["workstationClusters"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'workstationClusters'")
		} else {
...
func deleteResourceWorkstationsWorkstationCluster(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://workstations.googleapis.com/v1beta/projects/{{project}}/locations/{{location}}/workstationClusters/{{workstation_cluster_id}}"
