...
func TestAccNetworkManagementOrganizationVpcFlowLogsConfig_networkManagementOrgVpcFlowLogsConfigBasicExample(t *testing.T) {
	...
	acctest.VcrTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNetworkManagementOrganizationVpcFlowLogsConfig_networkManagementOrgVpcFlowLogsConfigBasicExample(context),
			},
			{
...
func testAccCheckNetworkManagementOrganizationVpcFlowLogsConfigDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			if rs.Type != "google_network_management_organization_vpc_flow_logs_config" {
				continue
			}
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, fmt.Sprintf("%s%s", transport_tpg.BaseUrl(networkmanagement.Product, config), "organizations/{{organization}}/locations/{{location}}/vpcFlowLogsConfigs/{{vpc_flow_logs_config_id}}"))
			if err != nil {
				return err
			}
			...
			if err == nil {
				return fmt.Errorf("NetworkManagementOrganizationVpcFlowLogsConfig still exists at %s", url)
			}
		}
