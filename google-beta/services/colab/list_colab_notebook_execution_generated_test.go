...
func TestAccColabNotebookExecutionListQuery_generated(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_colab_notebook_execution.notebook-execution",
						[]string{
							"notebook_execution_job_id",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_colab_notebook_execution.notebook-execution",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccColabNotebookExecution_colabNotebookExecutionBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_colab_notebook_execution.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_colab_notebook_execution.list_query",
...
func testAccColabNotebookExecution_colabNotebookExecutionBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_colab_notebook_execution" "list_query" {
    provider = google
	limit = 10000
    config {
