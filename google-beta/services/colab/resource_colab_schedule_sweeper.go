...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionColabSchedule,
		DeleteFunction: testSweepColabSchedule,
	}
...
func testSweepColabSchedule(_ string) error {
	return listAndActionColabSchedule(deleteResourceColabSchedule)
}

func listAndActionColabSchedule(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "ColabSchedule"
	...
	for _, mockConfig := range configs {
		...
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'schedules'")
		} else {
...
func deleteResourceColabSchedule(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
