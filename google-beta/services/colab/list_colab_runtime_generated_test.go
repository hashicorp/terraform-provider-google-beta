...
func TestAccColabRuntimeListQuery_generated(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					listDisplayName.Capture(
						"google_colab_runtime.runtime",
						[]string{
							"name",
						},
					),
					listScope.Capture(map[string]string{
						"location": "google_colab_runtime.runtime",
					}),
				),
			},
			{
				Query:           true,
				Config:          testAccColabRuntime_colabRuntimeBasicExampleListQuery(context),
				ConfigVariables: listScope.AsConfigVariables(),
				QueryResultChecks: []querycheck.QueryResultCheck{
					querycheck.ExpectLengthAtLeast("google_colab_runtime.list_query", 1),
					querycheck.ExpectResourceDisplayName(
						"google_colab_runtime.list_query",
...
func testAccColabRuntime_colabRuntimeBasicExampleListQuery(context map[string]interface{}) string {
	return acctest.Nprintf(`
variable "location" { type = string }
list "google_colab_runtime" "list_query" {
    provider = google
	limit = 10000
    config {
