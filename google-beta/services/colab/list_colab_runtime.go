...
func init() {
	registry.FrameworkListResource{
		...
		ProductName: "Colab",
		Func:        NewColabRuntimeListResource,
	}.Register()
}

var _ list.ListResource = &ColabRuntimeListResource{}

type ColabRuntimeListResource struct {
	tpgresource.ListResourceMetadata
}

func NewColabRuntimeListResource() list.ListResource {
	listR := &ColabRuntimeListResource{}
	listR.TypeName = "google_colab_runtime"
	listR.SDKv2Resource = ResourceColabRuntime()
...
// ColabRuntimeListModel matches ListResourceMetadata.ListConfigFields (tfsdk names and types).
type ColabRuntimeListModel struct {
	Location types.String `tfsdk:"location"`
	Project  types.String `tfsdk:"project"`
}

func (listR *ColabRuntimeListResource) List(ctx context.Context, listReq list.ListRequest, stream *list.ListResultsStream) {
	var data ColabRuntimeListModel
...
func ListColabRuntimes(config *transport_tpg.Config,
	location string,
	project string,
	callback func(*schema.ResourceData) error,
) error {
	resourceData := ResourceColabRuntime().Data(&terraform.InstanceState{})
	if location != "" {
	...
	url, err := tpgresource.ReplaceVars(resourceData, config, "{{ColabBasePath}}projects/{{project}}/locations/{{location}}/notebookRuntimes")
	if err != nil {
		return err
	}
	...
	return transport_tpg.ListPages(transport_tpg.ListPagesOptions{
		...
		Resource:       ResourceColabRuntime(),
		ListURL:        url,
		BillingProject: billingProject,
		UserAgent:      userAgent,
		ItemName:       "notebookRuntimes",
		Flattener: func(res map[string]interface{}, d *schema.ResourceData, config *transport_tpg.Config) error {
			...
			if res == nil {
				return fmt.Errorf("error decoding ColabRuntime from list response")
			}
			...
			if err = ResourceColabRuntimeFlatten(d, config, res, config, project, userAgent, billingProject, url, headers); err != nil {
				return err
			}
