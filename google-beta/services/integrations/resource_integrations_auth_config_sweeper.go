...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionIntegrationsAuthConfig,
		DeleteFunction: testSweepIntegrationsAuthConfig,
	}
...
func testSweepIntegrationsAuthConfig(_ string) error {
	return listAndActionIntegrationsAuthConfig(deleteResourceIntegrationsAuthConfig)
}

func listAndActionIntegrationsAuthConfig(action sweeper.ResourceAction) error {
	var lastError error
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://integrations.googleapis.com/v1/projects/{{project}}/locations/{{location}}/authConfigs", "?")[0]
...
func deleteResourceIntegrationsAuthConfig(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
