...
func init() {
	...
	s := &sweeper.Sweeper{
		...
		ListAndAction:  listAndActionBigqueryReservationReservationGroup,
		DeleteFunction: testSweepBigqueryReservationReservationGroup,
	}
...
func testSweepBigqueryReservationReservationGroup(_ string) error {
	return listAndActionBigqueryReservationReservationGroup(deleteResourceBigqueryReservationReservationGroup)
}

func listAndActionBigqueryReservationReservationGroup(action sweeper.ResourceAction) error {
	var lastError error
	resourceName := "BigqueryReservationReservationGroup"
	...
	for _, mockConfig := range configs {
		...
		// Prepare list URL
		listTemplate := strings.Split("https://bigqueryreservation.googleapis.com/v1/projects/{{project}}/locations/{{location}}/reservationGroups", "?")[0]
		...
		resourceList, ok := res["reservationGroups"]
		if ok {
			log.Printf("[INFO][SWEEPER_LOG] Found resources under expected key 'reservationGroups'")
		} else {
...
func deleteResourceBigqueryReservationReservationGroup(config *transport_tpg.Config, d *tpgresource.ResourceDataMock, obj map[string]interface{}) error {
	...
	deleteTemplate := "https://bigqueryreservation.googleapis.com/v1/projects/{{project}}/locations/{{location}}/reservationGroups/{{name}}"
