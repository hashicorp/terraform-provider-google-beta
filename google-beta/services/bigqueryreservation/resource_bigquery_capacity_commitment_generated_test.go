...
resource "google_bigquery_capacity_commitment" "commitment" {
  capacity_commitment_id = "tf-test-capacity-%{random_suffix}"
...
func testAccCheckBigqueryReservationCapacityCommitmentDestroyProducer(t *testing.T) func(s *terraform.State) error {
	return func(s *terraform.State) error {
		for name, rs := range s.RootModule().Resources {
			...
			url, err := tpgresource.ReplaceVarsForTest(config, rs, transport_tpg.BaseUrl(bigqueryreservation.Product, config)+"{{name}}")
			if err != nil {
				return err
			}
