...
func adjustInstanceFromMachineImageDisks(d *schema.ResourceData, config *Config, mi *computeBeta.MachineImage, zone *compute.Zone, project string) ([]*computeBeta.AttachedDisk, error) {
	...
	if _, hasBootDisk := d.GetOk("boot_disk"); hasBootDisk {
		...
		for _, disk := range mi.SourceInstanceProperties.Disks {
			if disk.Boot {
				newdisk := &computeBeta.AttachedDisk{
					Source:     disk.Source,
