...
func resourceComputeBackendBucketSignedUrlKeyRead(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := replaceVars(d, config, "https://www.googleapis.com/compute/beta/projects/{{project}}/global/backendBuckets/{{backend_bucket}}")
	if err != nil {
		return err
	}
...
func resourceComputeBackendBucketSignedUrlKeyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := replaceVars(d, config, "https://www.googleapis.com/compute/beta/projects/{{project}}/global/backendBuckets/{{backend_bucket}}/deleteSignedUrlKey?keyName={{name}}")
	if err != nil {
		return err
	}
...
func flattenNestedComputeBackendBucketSignedUrlKey(d *schema.ResourceData, meta interface{}, res map[string]interface{}) (map[string]interface{}, error) {
	...
	for _, vRaw := range items {
		...
		expectedV, err = expandComputeBackendBucketSignedUrlKeyName(d.Get("name"), d, meta.(*Config))
		if err != nil {
			return nil, err
		}
		itemV = flattenComputeBackendBucketSignedUrlKeyName(item["keyName"], d)
		log.Printf("[DEBUG] Checking if item's keyName (%#v) is equal to resource's (%#v)", itemV, expectedV)
