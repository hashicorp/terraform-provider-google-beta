...
* container: `data.google_container_engine_versions` supports `version_prefix` to allow fuzzy version matching. Using this field, Terraform can match the latest version of a major, minor, or patch release. [GH-506]
* pubsub: `google_pubsub_subscription` now supports configuring `message_retention_duration` and `retain_acked_messages`. [GH-503]
...
* storagetransfer: `google_storage_transfer_job` will no longer crash when accessing nil dates. [GH-499]
