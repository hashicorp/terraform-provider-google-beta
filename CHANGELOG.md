...
* storage: fixed issue where plans with a dependency on the `content` field could erroneously fail  in `google_storage_bucket_object_content` data source ([#7667](https://github.com/hashicorp/terraform-provider-google-beta/pull/7667))
* vertexai: fixed issue where updating "metadata" field failed in `google_vertex_ai_index` resource ([#7675](https://github.com/hashicorp/terraform-provider-google-beta/pull/7675))
